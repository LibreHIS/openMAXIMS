...
import ims.core.domain.CaseNoteFolderCc;
import ims.core.domain.PatientCaseNotes;
import ims.core.domain.PdsAddressSearch;
...
import ims.core.patient.domain.objects.PDSGeneralUpdateRequest;
...
public class PDSDemographicsImpl extends DTODomainImplementation implements ims.core.domain.PDSDemographics, ims.domain.impl.Transactional
{
	...
	private ims.core.vo.Patient replicatePatient(ims.core.vo.Patient voPatient) throws StaleObjectException, UniqueKeyViolationException, DomainInterfaceException
	{
		...
		return savePatientRecord(getDomainFactory(), voPatient, 0, false, true, null,null);
	}
	...
	public Patient savePatient(Patient patient, Boolean fromInterface, Boolean triggerPdsUpdate) throws UniqueKeyViolationException, StaleObjectException, DomainInterfaceException, IndexOutOfBoundsException
	{
		//WDEV-22448
		boolean markedDeceasedOnEdit = patient.getDodIsNotNull() && patient.getID_PatientIsNotNull();
		return savePatient(patient, fromInterface, markedDeceasedOnEdit, triggerPdsUpdate);
	}
	
	private Patient savePatient(Patient patient, Boolean fromInterface, Boolean patientDodSaved, Boolean triggerPdsUpdate) throws UniqueKeyViolationException, StaleObjectException, DomainInterfaceException, IndexOutOfBoundsException
	{
		DomainFactory factory = getDomainFactory();
		...
		return this.savePatientRecord(factory, patient, 1, fromInterface, patientDodSaved, triggerPdsUpdate);
	}	
	//WDEV-19715 --- end
	
	// wdev-7328 - default is not from dtoCall
	private Patient savePatientRecord(DomainFactory factory, Patient patient, int duplicateCount, boolean fromInterface, Boolean patientDodSaved, Boolean triggerPdsUpdate) throws StaleObjectException, UniqueKeyViolationException, DomainInterfaceException
	{
		return savePatientRecord(factory, patient, duplicateCount, fromInterface, false,  patientDodSaved, triggerPdsUpdate);
	}
	private Patient savePatientRecord(DomainFactory factory, Patient patient, int duplicateCount, boolean fromInterface, boolean fromDtoReplicate, Boolean patientDodSaved, Boolean triggerPdsUpdate) throws StaleObjectException, UniqueKeyViolationException, DomainInterfaceException
	{
		//WDEV-18245		
		...
		Boolean decoupled = patient.getDecoupled();
		PDSGeneralUpdateRequest domPDSGeneralUpdateRequest = null;
		if(Boolean.TRUE.equals(triggerPdsUpdate) && !Boolean.TRUE.equals(decoupled)/* && !hasPendingRelatives*/)
		{
			if (patient !=null && patient.getPDSPatientUpdate()!=null && patient.getPDSPatientUpdate().getPDSUpdateRequest()!=null)
			{    			
    			domPDSGeneralUpdateRequest = PDSGeneralUpdateRequestVoAssembler.extractPDSGeneralUpdateRequest(factory, patient.getPDSPatientUpdate().getPDSUpdateRequest());
			}
		}
		...
		try
		{
			...
			// WDEV-21883 - we don't want to set the verified number to false for PDS patients
			if ((!fromDtoReplicate || (domPatient.getIdentifiers().size() != patient.getIdentifiers().size())) 
					&& (!patient.getSourceOfInformationIsNotNull() || (patient.getSourceOfInformationIsNotNull() && !patient.getSourceOfInformation().equals(RegistrationSourceOfInfo.PDS))))
			{
			...
			if (ConfigFlag.UI.USE_HEARTS_CONTRACTING.getValue() && !fromInterface)
			{
				...
				if (preSavedPatient != null)
				{
					//compare postcodes WDEV-19684
					attendenceRecode = (savedPatientPostcode != null && patient.getAddress() != null) ? !savedPatientPostcode.equals(patient.getAddress().getPostCode()):(patient.getAddressesIsNotNull() ? patient.getAddress().getPostCodeIsNotNull() : false);					
					...
					{	 
						if(!voPatFilter.getID_Patient().equals(domPatient.getId()))  
						{
							...
							if (ConfigFlag.GEN.ALLOW_DUPLICATE_NHS_NUMBERS.getValue() && voPatId.getType().equals(PatIdType.NHSN))
							{
								...
								// Try the save again
								Patient newPat = savePatientRecord(factory, patient, ++duplicateCount, fromInterface, patientDodSaved, triggerPdsUpdate);
								
								try
								{
									PdsRetrievalQueryHelper helper = new PdsRetrievalQueryHelper(this);
									
									helper.setPatientInContext(newPat);
									helper.setNhsNumber(voPatId.getIdValue());
									
									PDSBackOfficeItemVo backOffice = helper.buildBackOfficeItemAndNotification(PDSBackOfficeType.DUPLICATE,  "A patient record with duplicated NHS Number was created.", PDSBackOfficeWorkPriority.P1);
									backOffice.validate();
									PDSBackOfficeItem domObj = PDSBackOfficeItemVoAssembler.extractPDSBackOfficeItem(factory, backOffice);
									factory.save(domObj);
								}
								catch (PdsException e1)
								{
									...
									throw new DomainInterfaceException(err);
								}
		...
		//check whether there are PdsRelative records with status pending 
		/*
		...
		if(voPatient.getPDSrelativesIsNotNull())
		{
			...
			{
				if(Boolean.TRUE.equals(voPatient.getPDSrelatives().get(i).getPending()))
				{
		...
		*/
		...
		//pending relatives will not be sent to PDS
		if(Boolean.TRUE.equals(triggerPdsUpdate) && !Boolean.TRUE.equals(decoupled)/* && !hasPendingRelatives*/)
		{
			if (domPDSGeneralUpdateRequest!=null)
			{
				domPDSGeneralUpdateRequest.setPatient(domPatient);
				factory.save(domPDSGeneralUpdateRequest);	
			}    			
		}
		
		if (newPatient)
		{
			PatientCaseNoteVo record = createAutomatedCaseNote(voPatient);
			if(record != null)
			{
...
//		apptColl.Appt_dt = DTOHelper.convertToDtoDate(patient.getDod().getDate());
	...
	//WDEV-23059
	private void removeFutureAppointmentDetailRecord(Patient patient) throws StaleObjectException
	{
		if (patient == null)
			return;
		
		String hql = "select futureAppt from FutureAppointmentDetails as futureAppt left join futureAppt.currentStatus as cStatus left join futureAppt.referral as cats left join cats.patient as pat where cStatus.pendingStatus.id = :OPEN and pat.id = :PATIENT_ID";
		
		DomainFactory domainFactory = getDomainFactory();
		List resultsToRemove = domainFactory.find(hql, new String[] {"OPEN", "PATIENT_ID"}, new Object[]{FutureAppointmentStatus.OPEN.getID(), patient.getID_Patient()});
		
		if (resultsToRemove == null || resultsToRemove.isEmpty())
			return;
		
		 Date removalDate = patient.getDod();	
		 Iterator it = resultsToRemove.iterator();
		
		 while (it.hasNext())
		 {
			 FutureAppointmentDetails item = (FutureAppointmentDetails) it.next();
			 item = removeFutureAppointment(item,removalDate);
			 domainFactory.save(item);
		 }		
	}
	...
	@Override
	public Integer countPendingPdsGeneralUpdatesRequests(PatientRefVo pat)
	{
		...
		markers.add("status");
		values.add(PDSSearchStatus.PENDING.getId());
	...
	@Override
	public PatientCaseNoteListVoCollection listCaseNoteFolders(PatientRefVo patient, String types, String statuses)
	{
		CaseNoteFolderCc impl = (CaseNoteFolderCc) getDomainImpl(CaseNoteFolderCcImpl.class);
		return impl.listCaseNoteFolders(patient, types, statuses);
	}
	...
	{
		PdsAddressSearch impl = (PdsAddressSearch) getDomainImpl(PdsAddressSearchImpl.class);
	...
	{
		boolean blankDataDetected = false;
		boolean hasObjectIdentifier = false;
		int notempty = 0;
		
		PdsRequestParameters params = new PdsRequestParameters(samlXml, appRole);
		
		/*  WDEV-22027
		 *  If any mappings are missing from MAXIMS in the context of PDS: 
            1. Set the Local patient SCN to 0 and terminate any other local updates for this PDS retrieval. 
            2. A Back Office issue should be created of type General with a description of the mapping type and value that is missing and priority 1. 
 		 */
		if(missingMappings != null && missingMappings.size() > 0)
		{
			lpiPatient.setSCN(0);
			
			String[] patErrors = lpiPatient.validate();
			if (patErrors != null)
			{
				for(int i = 0; i < patErrors.length; i++)
					errors.add(patErrors[i]);
			...
			try
			{
				lpiPatient = savePatient(lpiPatient, false, false);
			}
			catch (DomainInterfaceException e)
			{
				e.printStackTrace();
				errors.add("Cannot update LPI with PDS record !");
				return null;
			}
			catch (UniqueKeyViolationException e)
			{
			...
			{
				e.printStackTrace();
				errors.add("Data has been modified by another user - cannot update LPI with PDS record !");
				return null;
			}
			
			//create backoffice item
			PdsRetrievalQueryHelper helper = new PdsRetrievalQueryHelper(this);
			try
			{
				DomainFactory factory = getDomainFactory();
				
				helper.setPatientInContext(lpiPatient);
				helper.setNhsNumber(lpiPatient.getNhsn() != null ? lpiPatient.getNhsn().getIdValue() : "");
				PDSBackOfficeItemVo item = helper.buildBackOfficeItemAndNotification(PDSBackOfficeType.GENERAL, helper.getMissingMappingsAsText(missingMappings), PDSBackOfficeWorkPriority.P1);
				
		        String[] err = item.validate();
		        if(err != null && err.length > 0)
		        {
		        	throw new PdsException("Validation errors while creating a BackOfficeItem: " + err[0]);
		        }
		...
		lpiPatient.setPDSDateLastChecked(new Date());
		
		//WDEV-21226 - check name for blank data
		if(pdsPatient.getName() != null)
		{
			notempty = 0;
			
			if(pdsPatient.getName().getSurnameIsNotNull() && pdsPatient.getName().getSurname().length() > 0)
				notempty++;
			if(pdsPatient.getName().getForenameIsNotNull() && pdsPatient.getName().getForename().length() > 0)
				notempty++;
			if(pdsPatient.getName().getMiddleNameIsNotNull() && pdsPatient.getName().getMiddleName().length() > 0)
				notempty++;

			
			if(notempty > 0)
			{
				//overwrite local data as no blank data was detected
				lpiPatient.setName(pdsPatient.getName());
			}
			else
			{
				//blank data
				blankDataDetected = true;
				hasObjectIdentifier = lpiPatient.getNameIsNotNull() && lpiPatient.getName().getObjectidentifierIsNotNull() && lpiPatient.getName().getObjectidentifier().length() > 0; 

				if(!hasObjectIdentifier)
				{
					//overwrite local data because it doesn't have an object identifier
					lpiPatient.setName(pdsPatient.getName());
				}
			}
		}

		//WDEV-21226 - check address for blank data
		if(pdsPatient.getAddress() != null)
		{
			notempty = 0;
			
			if(pdsPatient.getAddress().getLine1IsNotNull() && pdsPatient.getAddress().getLine1().length() > 0)
				notempty++;
			if(pdsPatient.getAddress().getLine2IsNotNull() && pdsPatient.getAddress().getLine2().length() > 0)
				notempty++;
			if(pdsPatient.getAddress().getLine3IsNotNull() && pdsPatient.getAddress().getLine3().length() > 0)
				notempty++;
			if(pdsPatient.getAddress().getLine4IsNotNull() && pdsPatient.getAddress().getLine4().length() > 0)
				notempty++;
			if(pdsPatient.getAddress().getLine5IsNotNull() && pdsPatient.getAddress().getLine5().length() > 0)
				notempty++;

			if(notempty > 0)
			{
				//overwrite local data as no blank data was detected
				lpiPatient.setAddress(pdsPatient.getAddress());
			}
			else
			{
				//blank data
				blankDataDetected = true;
				hasObjectIdentifier = lpiPatient.getAddressIsNotNull() && lpiPatient.getAddress().getObjectidentifierIsNotNull() && lpiPatient.getAddress().getObjectidentifier().length() > 0; 
		...
		if(lpiPatient.getCommChannels() == null)
			lpiPatient.setCommChannels(pdsPatient.getCommChannels());
		else
		{
			//we need to remove commchannels that have no object identifier and have an update mode = ADDED that are in the collection returned by PDS.
			ArrayList<CommChannelVo> tbr = new ArrayList<CommChannelVo>();
			
			for(CommChannelVo cc : lpiPatient.getCommChannels())
			{
				if(cc.getObjectidentifier()==null && (!cc.getPdsUpdateModeIsNotNull() || (cc.getPdsUpdateModeIsNotNull() && cc.getPdsUpdateMode().equals(PDSUpdateMode.ADDED))))
				{
					for(CommChannelVo ccpds : pdsPatient.getCommChannels())
					{
						if ((ccpds.getChannelTypeIsNotNull() && cc.getChannelTypeIsNotNull()) && (ccpds.getChannelType().equals(cc.getChannelType())))
						{
							//WDEV-22114 - WDEV-21985
							//WDEV-22186
							if ((ccpds.getCommValueIsNotNull() && cc.getCommValueIsNotNull()) 
									&& (ccpds.getCommValue().replaceAll(" ", "").toUpperCase().equals(cc.getCommValue().toUpperCase())))
								tbr.add(cc);
							//WDEV-22186							
						}
					}
			...
			//also remove any that are held locally with object identifier before adding form pds again 
			for(CommChannelVo cc : lpiPatient.getCommChannels())
			{
			...
			for(CommChannelVo cc : tbr)
			{
				for(CommChannelVo ccPds : pdsPatient.getCommChannels())
				{
					if (ccPds.getObjectidentifierIsNotNull() && ccPds.getObjectidentifier().equals(cc.getObjectidentifier()))
					{
						if (ccPds.getCommValueIsNotNull() && !ccPds.getCommValue().equals(""))
						{
							//safe to remove for local update form pds
							lpiPatient.getCommChannels().remove(cc);
						}
					}
				}				
			}
			...
			//now add what was retrieved from PDS if not blank
			for(CommChannelVo cc : pdsPatient.getCommChannels())
			{
				if (cc.getCommValueIsNotNull() && !cc.getCommValue().equals(""))
					lpiPatient.getCommChannels().add(cc);
				else
					blankDataDetected = true;
			}
		}

		lpiPatient.setOtherNames(pdsPatient.getOtherNames());
		lpiPatient.setAddresses(pdsPatient.getAddresses());
		
		lpiPatient.setSCN(blankDataDetected ? 0 : pdsPatient.getSCN());

		lpiPatient.setSex(pdsPatient.getSex());
		lpiPatient.setDob(pdsPatient.getDob());

		// WDEV-21930 If Date of death is null and local one was not null, need to clear the status too
		if (lpiPatient.getDodIsNotNull() && pdsPatient.getDob() == null)
		{
			lpiPatient.setPDSDeathNotificationStatus(null);
		}	
		lpiPatient.setDod(pdsPatient.getDod());

		//WDEV-22199
		//If the currently assigned local GP's practice is Not the same as the returned practice (practicecode) then we clear the patient gp record.													
		if(lpiPatient.getPracticeIsNotNull() && !lpiPatient.getPractice().equals(pdsPatient.getPractice()))
		{
			//clear GP selection
			lpiPatient.setGp(null);
		}
		//WDEV-22199
		
		lpiPatient.setGpSurgery(pdsPatient.getGpSurgery());
		
		// WDEV-21930 - Update Patient GP within the object itself if needed as otherwise a new PDSPAtientGP record will be created
		if (lpiPatient.getPDSPatientGP() == null)
		{
			lpiPatient.setPDSPatientGP(pdsPatient.getPDSPatientGP());
		}
		else if (lpiPatient.getPDSPatientGPIsNotNull() && !pdsPatient.getPDSPatientGPIsNotNull())
		{
			lpiPatient.setPDSPatientGP(null);
		}
		else
		{
			lpiPatient.getPDSPatientGP().setBeffdate(pdsPatient.getPDSPatientGP().getBeffdate());
			lpiPatient.getPDSPatientGP().setBeffdatecnf(pdsPatient.getPDSPatientGP().getBeffdatecnf());
			lpiPatient.getPDSPatientGP().setBetdatecnf(pdsPatient.getPDSPatientGP().getBetdatecnf());
			lpiPatient.getPDSPatientGP().setGp(pdsPatient.getPDSPatientGP().getGp());
			lpiPatient.getPDSPatientGP().setLkp_provisiontype(pdsPatient.getPDSPatientGP().getLkp_provisiontype());
			lpiPatient.getPDSPatientGP().setObjectidentifier(pdsPatient.getPDSPatientGP().getObjectidentifier());
			lpiPatient.getPDSPatientGP().setPractice(pdsPatient.getPDSPatientGP().getPractice());
			lpiPatient.getPDSPatientGP().setPrimaryCareIdentifier(pdsPatient.getPDSPatientGP().getPrimaryCareIdentifier());
			
		}
		
		//patient identifiers
		//WDEV-21382
		if(lpiPatient.getIdentifiers() == null)
			lpiPatient.setIdentifiers(new PatientIdCollection());
		//else
		//why are we clearing identifiers?
			//lpiPatient.getIdentifiers().clear();
			
// Don't need to do this. 
		if(pdsPatient.getIdentifiers() != null )
		{
			for(int i = 0; i < pdsPatient.getIdentifiers().size(); i++)
			{
				boolean lFoundNhsNumber = false;
				if(!Boolean.TRUE.equals(pdsPatient.getIdentifiers().get(i).getSuperseeded()))
				{
					//WDEV-21382
					//update the existing identifier type nhs number
					for(int j = 0; j < lpiPatient.getIdentifiers().size(); j++)
					{
						//WDEV-21414 //SN
						// WDEV-21948 - Check the value and type to see if we have the NHS number BW
						if (lpiPatient.getIdentifiers().get(j).getTypeIsNotNull() && pdsPatient.getIdentifiers().get(i).getTypeIsNotNull() 
								&&  pdsPatient.getIdentifiers().get(i).getType().equals(lpiPatient.getIdentifiers().get(j).getType())
								&& lpiPatient.getIdentifiers().get(j).getValueIsNotNull() && pdsPatient.getIdentifiers().get(i).getValueIsNotNull()
								&& pdsPatient.getIdentifiers().get(i).getValue().equals(lpiPatient.getIdentifiers().get(j).getValue())
								)
						{
							//update nhs number
							lFoundNhsNumber = true;
							//WDEV-21392 - SN
							lpiPatient.getIdentifiers().get(j).setVerified(true);
							pdsPatient.getIdentifiers().get(i).setVerified(true);  // WDEV-21883 - on update, if NHS number there, need to set verified to true
							break;
						}
					}
				}
				...
				{
					// WDEV-21948 - This identifier has been superseded so we should set verified to false
					for(int j = 0; j < lpiPatient.getIdentifiers().size(); j++)
					{
						if (lpiPatient.getIdentifiers().get(j).getTypeIsNotNull() && pdsPatient.getIdentifiers().get(i).getTypeIsNotNull() 
								&&  pdsPatient.getIdentifiers().get(i).getType().equals(lpiPatient.getIdentifiers().get(j).getType())
						...
						{
							lpiPatient.getIdentifiers().get(j).setVerified(false);
							lFoundNhsNumber=true;
							break;
						}
					}
				...
				//Changed on behalf of SN
				if (!lFoundNhsNumber)
					lpiPatient.getIdentifiers().add(pdsPatient.getIdentifiers().get(i));
		...
		{
			for(int i = 0; i < pdsPatient.getPDSrelatives().size(); i++)
			{
				PatRelative rel = pdsPatient.getPDSrelatives().get(i);
				...
				if(rel.getNhsNumIsNotNull() && rel.getNhsNum().length() > 0)
				{
    				...
    				catch (PdsException e)
    				{
    					...
    					errors.add("Error calling PdsRetrievalQuery: " + e.toString());
					...
					else
					{
						...
						if(lpiPatient.getPDSrelatives() == null)
						{
							lpiPatient.setPDSrelatives(new PatRelativeCollection());
							newRel = new PatRelative();
							lpiPatient.getPDSrelatives().add(newRel);
						}
						else
						{
							newRel = getRelativeByObjectIdentifier(lpiPatient.getPDSrelatives(), rel.getObjectidentifier());
							
							//WDEV-22172
							if(newRel == null)
							{
								newRel = getRelativeByRelativeRecord(lpiPatient.getPDSrelatives(), rel);								
								if(newRel == null)
								{								
									newRel = new PatRelative();
									lpiPatient.getPDSrelatives().add(newRel);
						...
						newRel.setPending(Boolean.TRUE);
		...
		{
			lpiPatient.setIsConfidential(Boolean.TRUE);
			
			if(pdsPatient.getAddress() == null)
			{
				lpiPatient.setAddress(null);
				lpiPatient.setAddresses(null);
			}
		}
		
		if(pdsPatient.getAlternativeContact() != null)
		{
			if(lpiPatient.getAlternativeContact() == null)
				lpiPatient.setAlternativeContact(pdsPatient.getAlternativeContact());
			else
			{
				PDSConsentHistoryVoCollection historyColl = lpiPatient.getAlternativeContact().getConsentHistory();
				if(historyColl == null)
				{
					historyColl = new PDSConsentHistoryVoCollection();
					lpiPatient.getAlternativeContact().setConsentHistory(historyColl);
				}
				
				//check whether any of the consent history fields have changed
				boolean consentHistoryChanged = false;
				if(lpiPatient.getAlternativeContact().getPdsDateLastChanged() != null ? !lpiPatient.getAlternativeContact().getPdsDateLastChanged().equals(pdsPatient.getAlternativeContact().getPdsDateLastChanged()) : pdsPatient.getAlternativeContact().getPdsDateLastChanged() != null)
					consentHistoryChanged = true;
				if(lpiPatient.getAlternativeContact().getPdsConsent() != null ? !lpiPatient.getAlternativeContact().getPdsConsent().equals(pdsPatient.getAlternativeContact().getPdsConsent()) : pdsPatient.getAlternativeContact().getPdsConsent() != null)
					consentHistoryChanged = true;
				if(lpiPatient.getAlternativeContact().getPdsConsentType() != null ? !lpiPatient.getAlternativeContact().getPdsConsentType().equals(pdsPatient.getAlternativeContact().getPdsConsentType()) : pdsPatient.getAlternativeContact().getPdsConsentType() != null)
					consentHistoryChanged = true;
				...
				if(consentHistoryChanged)
				{
					...
					consentHistory.setComment(lpiPatient.getAlternativeContact().getComment());
					consentHistory.setConsent(lpiPatient.getAlternativeContact().getPdsConsent());
					consentHistory.setConsentType(lpiPatient.getAlternativeContact().getPdsConsentType());
					consentHistory.setDate(lpiPatient.getAlternativeContact().getPdsDateLastChanged());
					
					historyColl.add(consentHistory);
				}
				
				lpiPatient.getAlternativeContact().setPdsConsent(pdsPatient.getAlternativeContact().getPdsConsent());
				lpiPatient.getAlternativeContact().setPdsDateLastChanged(pdsPatient.getAlternativeContact().getPdsDateLastChanged());
				lpiPatient.getAlternativeContact().setPdsConsentType(pdsPatient.getAlternativeContact().getPdsConsentType());
				
				if(lpiPatient.getAlternativeContact().getPdsConsentType() == null)
				{
					lpiPatient.getAlternativeContact().setPdsConsentType(PDSConsentType.NHSCARERECORDSHARING);
				}
				
				lpiPatient.getAlternativeContact().setPdsPreferredContactMethod(pdsPatient.getAlternativeContact().getPdsPreferredContactMethod());
				lpiPatient.getAlternativeContact().setPdsPreferredContactTimes(pdsPatient.getAlternativeContact().getPdsPreferredContactTimes());
				lpiPatient.getAlternativeContact().setPdsPreferredWrittenComm(pdsPatient.getAlternativeContact().getPdsPreferredWrittenComm());
			}
		}
		
		lpiPatient.setPDSInterpreterRequired(pdsPatient.getPDSInterpreterRequired());
		lpiPatient.setPrefCommLanguage(pdsPatient.getPrefCommLanguage());
		
		//VP - we need to set the source of information to PDS as the patient is retrieved/updated from PDS
		lpiPatient.setSourceOfInformation(RegistrationSourceOfInfo.PDS);
		lpiPatient.setSCN(pdsPatient.getSCN());  // WDEV-21883
	...
	*/
	...
	@Override
	public SchoolCollection getSchool(String text)
	{
