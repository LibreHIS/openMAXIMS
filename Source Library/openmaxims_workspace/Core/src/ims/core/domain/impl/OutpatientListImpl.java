...
public class OutpatientListImpl extends BaseOutpatientListImpl
{
	...
	public CareContextShortVo getContextForPasEvent(PASEventRefVo pasEvent)
	{
		if (pasEvent == null || pasEvent.getID_PASEvent() == null)
			return null;
		
		String query = "SELECT context FROM CareContext AS context LEFT JOIN context.pasEvent AS pas WHERE pas.id = :PAS_ID";
		
		return CareContextShortVoAssembler.create((CareContext) getDomainFactory().findFirst(query, "PAS_ID", pasEvent.getID_PASEvent()));
	}
	...
	public PatientDocumentVo getPatientDocument(PatientDocumentRefVo documentID) throws StaleObjectException
	{
		...
		if(Boolean.TRUE.equals(domO.getIsRIE()))
			throw new StaleObjectException(domO);
