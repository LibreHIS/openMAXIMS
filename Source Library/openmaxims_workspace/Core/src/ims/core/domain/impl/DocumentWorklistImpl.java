...
public class DocumentWorklistImpl extends BaseDocumentWorklistImpl
{
	...
	private static final Integer GENERATED_DATE	= 1;
	private static final Integer EVENT_DATE	= 2;
	...
	public ims.core.vo.PatientDocumentforDocumentWorklistVoCollection listPatientDocuments(ims.core.vo.DocumentWorklistSearchCriteriaVo searchCriteria)
	{
		...
		{
			hqlConditions.append(and);
			if (EVENT_DATE.equals(searchCriteria.getSearchType()))
			{
				hqlConditions.append("pd.documentDate >= :dateFrom");
			}
			else if (GENERATED_DATE.equals(searchCriteria.getSearchType()))
			{
				hqlConditions.append(" pd.systemInformation.creationDateTime >= :dateFrom ");
			}
		...
		if (searchCriteria.getDateToIsNotNull())
		{
			...
			{
				hqlConditions.append(" pd.systemInformation.creationDateTime < :dateTo ");
			}
		...
		if (searchCriteria.getPatientIdIsNotNull())
		{
			...
			{
				searchCriteria.getPatientId().setValue(searchCriteria.getPatientId().getValue().toUpperCase());
			}
			...
			{
				paramValues.add(searchCriteria.getPatientId().getValue().trim());
			}
