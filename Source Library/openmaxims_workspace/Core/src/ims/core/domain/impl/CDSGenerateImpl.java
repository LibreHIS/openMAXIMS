...
import ims.RefMan.domain.objects.ReviewPatientElectiveList;
...
import ims.core.cds.domain.objects.CDSElectiveList;
...
import ims.core.cds.domain.objects.CDSWaitingListLocGroup;
...
import ims.core.vo.CDSElectiveListVo;
...
import ims.core.vo.CDSWaitingListLocGroupVo;
...
public class CDSGenerateImpl extends BaseCDSGenerateImpl implements StatefulJob
{
	...
	private static final int INVESTIGATION_TYPE=3;
	...
	private static String		ELECTIVELIST_REPORT		= "CDS ElectiveList"; //WDEV-18912
	...
	public synchronized void doExecute(JobExecutionContext ctx, Date last, Date curr) throws Exception 
	{
		...
		try 
		{
			...
			if (jb != null )
			{
				...
				//WDEV-18912
				else if (jbType != null && jbType.equals("ELECTIVELIST"))
				{
					impl.createSystemLogEntry(SystemLogType.QUARTZ_JOB, SystemLogLevel.INFORMATION, "Elective List CDS Data Generate Started for Date Range (" + dateFrom + " to " + dateTo + ") ");

					int numRecords = impl.generateElectiveListCDSData(dateFrom, dateTo);
					java.util.Date stopTime = new java.util.Date();
					long totalTime = stopTime.getTime() - startTime.getTime();
					impl.createSystemLogEntry(SystemLogType.QUARTZ_JOB, SystemLogLevel.INFORMATION, "Elective List CDS Data Generated Successfully for Date Range (" + dateFrom + " to " + dateTo + ") " + numRecords + " records generated - took " +  formatTimeTaken(totalTime));
				} //WDEV-18912
				else if (jbType != null && (jbType.equals(OUTPATIENT_REPORT_TYPE) || jbType.equals(INPATIENT_REPORT_TYPE) || jbType.equals(HRG_REPORT_TYPE)))
				{
					...
					//WDEV-18912
					else if(jbType.equals(ELECTIVELIST_REPORT_TYPE))
					{
						rep = "ElectiveList";
						repId = ELECTIVELIST_REPORT_ID;
					} //WDEV-18912
	...
	public Integer generateOutpatCDSData(ims.framework.utils.Date fromDate, ims.framework.utils.Date toDate) throws ims.domain.exceptions.DomainInterfaceException, ims.domain.exceptions.StaleObjectException
	{
		...
		// WDEV-13368
		String deleteOutpatientSql = " delete from core_cdsprocdetail where core_cdsoutpatient_opcsproced in (select id from core_cdsoutpatient where core_cdsoutpatient.apptdate between ? and ?)";
		...
		{
			hql.append(" and appt.appointmentDate < :toDate"); //WDEV-19260 
			markers.add("toDate");
...
//			cds.setSiteCodeTreatment(formatStr(siteCode, (CDSOutpatientVo.getSiteCodeTreatmentMaxLength())));  
			...
			// WDEV-18917
			// Check if ReferralCoding has been performed for this appointment -- WDEV-19175  check isActive flag
			List codingList = factory.find(" select cod from ReferralCoding cod where (cod.active = true or cod.active is null) and cod.appointment = :appointment and (cod.isRIE is null OR cod.isRIE = false) ", new String[]{"appointment"}, new Object[]{domAppt});
			if (codingList != null && codingList.size() > 0)
			{
				for (int j=0; j<codingList.size(); j++)
				{
					
					ReferralCoding domRefCode = (ReferralCoding) codingList.get(j);
					...
					if (codeItems != null && codeItems.size() > 0)
					{
						...
						for (int x=0; x<codeItems.size(); x++)
						{
							...
							if (refItem.getPatientProcedure() != null)
							{
								...
								domProcDet.setProcedureDate(lastProcDate);  
								...
								cds.getOPCSProcedure().add(domProcDet);
							}
							else if (refItem.getInvestigation() != null)
							{
								lastType=INVESTIGATION_TYPE;
								CDSProcedureDetails domProcDet = new CDSProcedureDetails();
								domProcDet.setProcedureScheme("02");  // OPCS-4
								domProcDet.setProcedure(formatStr(refItem.getOPCS4(), CDSProcedureDetailsVo.getProcedureMaxLength()));
								
								lastInvProcDate = null;
								if (refItem.getInvestigation().getResultDetails() != null && refItem.getInvestigation().getResultDetails().getClinicalResultDetails() != null)
								{
									lastInvProcDate=refItem.getInvestigation().getResultDetails().getClinicalResultDetails().getExamDateTime();
									domProcDet.setProcedureDate(lastInvProcDate);  
								}
								
								cds.getOPCSProcedure().add(domProcDet);
							}
							else if (refItem.getPatientDiagnosis() != null)
							{
								...
								cds.getICDDiagGroup().add(domDiagDet);
							}
							else if (lastType == PROCEDURE_TYPE && refItem.getOPCS4() != null)
							{
								CDSProcedureDetails domProcDet = new CDSProcedureDetails();
								domProcDet.setProcedureScheme("02");  // OPCS-4
								domProcDet.setProcedure(formatStr(refItem.getOPCS4(), CDSProcedureDetailsVo.getProcedureMaxLength()));
							...
							else if (lastType == INVESTIGATION_TYPE && refItem.getOPCS4() != null)
							{
								CDSProcedureDetails domProcDet = new CDSProcedureDetails();
								domProcDet.setProcedureScheme("02");  // OPCS-4
								domProcDet.setProcedure(formatStr(refItem.getOPCS4(), CDSProcedureDetailsVo.getProcedureMaxLength()));
							...
							else if (lastType == DIAGNOSIS_TYPE && refItem.getICD10() != null)
							{
								CDSDiagnosisDetails domDiagDet = new CDSDiagnosisDetails();
								domDiagDet.setDiagScheme(formatStr("02", CDSDiagnosisDetailsVo.getDiagSchemeMaxLength()));  // 02=ICD
	...
	//WDEV-18912
	@SuppressWarnings("rawtypes")
	public Integer generateElectiveListCDSData(ims.framework.utils.Date fromDate, ims.framework.utils.Date toDate) throws ims.domain.exceptions.DomainInterfaceException, ims.domain.exceptions.StaleObjectException
	{
		DomainFactory factory = getDomainFactory();

		// Elective List details 
		String deleteElectiveListSql = "delete from core_cdselectivelis where core_cdselectivelis.startdate between ? and ?";
		deleteFromTable(deleteElectiveListSql, fromDate, toDate);
		...
		markers.add("BookedType");
		values.add(ElectiveAdmissionType.BOOKED_TYPE12.getId());
		markers.add("ElecType");
		values.add(ElectiveAdmissionType.ELECTIVE_TYPE11.getId());

		if (fromDate != null && toDate != null)
		{
			hql.append(" and pel.dateOnList >= :fromDate and pel.dateOnList < :toDate");
			markers.add("fromDate");
			markers.add("toDate");
		...
		{
			hql.append(" and pel.dateOnList >= :fromDate");
			markers.add("fromDate");
			values.add(new DateTime(fromDate).getJavaDate());
		}
		else if (fromDate == null && toDate != null)
		{
			hql.append(" and pel.dateOnList < :toDate");
			markers.add("toDate");
			values.add(new DateTime(toDate.addDay(1)).getJavaDate());
		}
		...
		Map<String, CDSElectiveList> electiveListMap = new HashMap<String, CDSElectiveList>();
		String[] errors = new String[lst.size()];
		int errorCounter = 0;
		
		// Loop through each PatientElectiveList object and process to create a CDSElectiveList object
		for(int i = 0; i < lst.size(); i++)
		{
			PatientElectiveList domPel = (PatientElectiveList) lst.get(i);
			LOG.debug("Record " + i + " Event " + domPel.getId());
			
			CDSElectiveList cds = CDSElectiveList.getCDSElectiveListFromElectiveListID(factory, domPel.getId().toString());
			if (cds == null)
			{
				cds = new CDSElectiveList();
			}
			
			//Set unique ID
			cds.setElectiveListID(domPel.getId().toString());
	
			//System information
			if(domPel.getSystemInformation() != null)
			{
				if(domPel.getSystemInformation().getLastUpdateDateTime() != null)
					cds.setCDSApplicableDate(domPel.getSystemInformation().getLastUpdateDateTime());
				else
					cds.setCDSApplicableDate(domPel.getSystemInformation().getCreationDateTime());
			}
			
			cds.setCDSSenderOrgCode(formatStr(defaultSiteCode, CDSElectiveListVo.getCDSSenderOrgCodeMaxLength()));

			//UBRN
			if(domPel.getReferral() != null
					&& domPel.getReferral().getICABReferral() != null
					&& domPel.getReferral().getICABReferral().getUBRN() != null)
			{
				cds.setUBRN(formatStr(domPel.getReferral().getICABReferral().getUBRN(), CDSElectiveListVo.getUBRNMaxLength()));
			}
			else
			{
				cds.setUBRN(formatStr(" ", CDSElectiveListVo.getUBRNMaxLength()));
			}

			//PathwayIdentifier
			// ?????????????????
			//			cds.setPathwayIdentifier(CONTRACT_CODE + formatDec(domPel.getId(), CDSInpatientVo.getPathwayIdentifierMaxLength()- CONTRACT_CODE.length()));
			if(domPel.getReferral() != null
					&& domPel.getReferral().getPathwayID() != null)
			{
				cds.setPathwayIdentifier(formatStr(domPel.getReferral().getPathwayID().toString(), CDSElectiveListVo.getPathwayIdentifierMaxLength()));
			}
			else
			{
				cds.setPathwayIdentifier(formatStr(" ", CDSElectiveListVo.getPathwayIdentifierMaxLength()));
			}

			//PathwayOrgCode
			//	The OrganisationPCTLinkConfig object can be used to influence what is output to the  Organisation Code for Provider and the Patient Id Organisation Code.
			//	If there is a configured record for the PCT value that’s currently associated with the Commissioner Organisation Code, its Organisation Code will be used for the two fields specified.
			if(domPel.getReferral() != null
					&& domPel.getReferral().isIsCAB())
			{
				cds.setPathwayOrgCode(formatStr("X09", CDSElectiveListVo.getPathwayOrgCodeMaxLength()));
			}
			else
			{
				cds.setPathwayOrgCode(formatStr(" ", CDSElectiveListVo.getPathwayOrgCodeMaxLength()));				
			}
			
			//ReferralToTreatmentStatus
			if(domPel.getReferral() != null
					&& domPel.getReferral().getCurrentRTTStatus() != null
					&& domPel.getReferral().getCurrentRTTStatus().getRTTStatus() != null
					&& domPel.getReferral().getCurrentRTTStatus().getRTTStatus().getNationalCode() !=null)
				cds.setReferralToTreatmentStatus(formatStr(domPel.getReferral().getCurrentRTTStatus().getRTTStatus().getNationalCode().toString(), CDSElectiveListVo.getReferralToTreatmentStatusMaxLength()));
						
			//WaitTimeMeasureType
			if(domPel.getReferral().getRTTClockImpacts() != null)
			{
				String waitTimeMeasureType = "02";
				List rttClockImpacts = domPel.getReferral().getRTTClockImpacts();
				if(rttClockImpacts != null
						&& rttClockImpacts.size() > 0)
				{
					waitTimeMeasureType = "01";
				}
				cds.setWaitTimeMeasureType(formatStr(waitTimeMeasureType, CDSElectiveListVo.getWaitTimeMeasureTypeMaxLength()));
			}
			else
			{
				cds.setWaitTimeMeasureType(formatStr(" ", CDSElectiveListVo.getWaitTimeMeasureTypeMaxLength()));
			}
			
			//RTTStartDate
			if(domPel.getReferral() != null
					&& domPel.getReferral().getJourney() != null
					&& domPel.getReferral().getJourney().getCurrentClock() != null
					&& domPel.getReferral().getJourney().getCurrentClock().getStartDate() != null)
				cds.setRTTStartDate(domPel.getReferral().getJourney().getCurrentClock().getStartDate());

			//RTTEndDate
			if(domPel.getReferral() != null
					&& domPel.getReferral().getJourney() != null
					&& domPel.getReferral().getJourney().getCurrentClock() != null
					&& domPel.getReferral().getJourney().getCurrentClock().getStopDate() != null)
				cds.setRTTEndDate(domPel.getReferral().getJourney().getCurrentClock().getStopDate());

			//CCGOrgCode
			if(domPel.getPatient() != null
					&& domPel.getPatient().getAddress() != null
					&& domPel.getPatient().getAddress().getPCT() != null)
			{
				cds.setCCGOrgCode(formatStr(domPel.getPatient().getAddress().getPCT(), CDSElectiveListVo.getCCGOrgCodeMaxLength()));
			}
			else
			{
				cds.setCCGOrgCode(formatStr(" ", CDSElectiveListVo.getCCGOrgCodeMaxLength()));
			}
				
			//WithHeldIdentityReason
			cds.setWithHeldIdentityReason(formatStr(" ", CDSElectiveListVo.getWithHeldIdentityReasonMaxLength()));

			
			String nhsNum="";
			PatientId domPatId = getNHSNumber(domPel.getPatient());
			if(domPatId != null)
				nhsNum = domPatId.getValue();
			Patient domPatient = domPel.getPatient();
			
			// If this is a dummy patient - we do not want to include it.
			if (domPatient != null 
					&& domPatient.getName() != null 
					&& domPatient.getName().getSurname().equals("DummyICABSurname"))
			{
				factory.evict(cds);
				errors[errorCounter++] = "DummyICAB Patient Found - will be ignored - " + domPatient.getId() + " (" + nhsNum + ")";
				continue;
			...
			cds.setLocalPatientIdentifier(formatStr(domPatient.getId(), CDSElectiveListVo.getLocalPatientIdentifierMaxLength()));

			//NHSNumber
			//NHSNumberStatusInd
			//PatientName (????? Patient.name (only output if NHS_NUMBER_STATUS_INDICATOR=07 ???????)
			//PatientAddress
			if (domPatId != null)
			{ 
				cds.setNHSNumber(domPatId.getValue());
				if (domPatId.isVerified())
				{
					...
					if(domPel.getReferral() != null
							&& domPel.getReferral().getPathwayID() != null)
						cds.setPatIdOrgCode(formatStr(domPel.getReferral().getPathwayID(), CDSElectiveListVo.getPatIdOrgCodeMaxLength()));
				}
				else
				{
					cds.setNHSNumberStatusInd("02");
				}
				cds.setPatientName(formatStr(" ", CDSElectiveListVo.getPatientNameMaxLength()));
				cds.setPatientAddress(formatStr(" ", CDSElectiveListVo.getPatientAddressMaxLength()));
			}
			else
			{
				cds.setNHSNumber(formatStr(" ", CDSElectiveListVo.getNHSNumberMaxLength()));
				cds.setNHSNumberStatusInd("09");  // Not Present
				cds.setPatientName(formatStr(domPatient.getName() == null ? " " : domPatient.getName().toString(), CDSElectiveListVo.getPatientNameMaxLength()));
				cds.setPatientAddress(formatStr(domPatient.getAddress() == null ? " " :domPatient.getAddress().toString(), CDSElectiveListVo.getPatientAddressMaxLength()));
			}

			//PostCode
			cds.setPostCode(formatStr(domPatient.getAddress() == null ? "":domPatient.getAddress().getPostCode(), CDSElectiveListVo.getPostCodeMaxLength()));

			//PCTOrgCode
			// ????????????????????????? The ORGANISATION CODE derived from the PATIENT's POSTCODE OF USUAL ADDRESS
			if(domPel.getPatient() != null
					&& domPel.getPatient().getAddress() != null
					&& domPel.getPatient().getAddress().getPCT() != null)
			{
				cds.setPCTOrgCode(formatStr(domPel.getPatient().getAddress().getPCT(), CDSElectiveListVo.getPCTOrgCodeMaxLength()));
			}
			else
			{
				cds.setPCTOrgCode(formatStr(" ", CDSElectiveListVo.getPCTOrgCodeMaxLength()));
			}
			...
			cds.setPersonGender(formatStr(gender, CDSElectiveListVo.getPersonGenderMaxLength()));

			//CareSupportInd
			cds.setCarerSupportInd(formatStr(" ",CDSElectiveListVo.getCarerSupportIndMaxLength()));

			
			
			// DATA GROUP: EAL SERVICE AGREEMENT CHANGE DETAILS
			//CommSerialNumber
			cds.setCommSerialNumber(formatStr(" ", CDSElectiveListVo.getCommSerialNumberMaxLength()));			
			//NHSServAgreeLineNum
			cds.setNHSServAgreeLineNum(formatStr(" ", CDSElectiveListVo.getNHSServAgreeLineNumMaxLength()));
			//ProviderRefNum
			cds.setProviderRefNum(formatStr(" ", CDSElectiveListVo.getProviderRefNumMaxLength()));
			//CommissionerReferenceNum
			cds.setCommissionerReferenceNum(formatStr(" ", CDSElectiveListVo.getCommissionerReferenceNumMaxLength()));
			//OrgCodeProvider ?????????????????????
			//	Configuration Flag Value CDS_DEFAULT_SITE_CODE or replacement.  See Appendix A. 
			//	The OrganisationPCTLinkConfig object can be used to influence what is output to the  Organisation Code for Provider and the Patient Id Organisation Code.
			//	If there is a configured record for the PCT value that’s currently associated with the Commissioner Organisation Code, its Organisation Code will be used for the two fields specified.
			cds.setOrgCodeProvider(formatStr(" ", CDSElectiveListVo.getOrgCodeProviderMaxLength()));
			//OrgCodeCommissioner
			if(domPel.getReferral() != null
					&& domPel.getReferral().getReferralDetails() != null
					&& domPel.getReferral().getReferralDetails().getPCT() != null)
			{
				cds.setOrgCodeCommissioner(formatStr(domPel.getReferral().getReferralDetails().getPCT(), CDSElectiveListVo.getOrgCodeCommissionerMaxLength()));
			}
			else
			{
				cds.setOrgCodeCommissioner(formatStr(" ", CDSElectiveListVo.getOrgCodeCommissionerMaxLength()));
			}
			...
			//ElectiveListId
			cds.setElectiveListID(formatStr(domPel.getId(), CDSElectiveListVo.getElectiveListIDMaxLength()));
			
			//AdminCategory
			if (domPel.getReferral() != null
					&& domPel.getReferral().getReferralCategory() != null)
			{
				cds.setAdminCategory(formatStr(getExternalMapping(domPel.getReferral().getReferralCategory(), "01"), CDSElectiveListVo.getAdminCategoryMaxLength()));
			}
			else
			{
				cds.setAdminCategory(formatStr(" ", CDSElectiveListVo.getAdminCategoryMaxLength()));
			}
			
			//CountSuspendedDays
			// COUNT OF DAYS SUSPENDED gives an aggregate count of the number of days the PATIENT has been suspended since either the LAST DNA OR PATIENT CANCELLED DATE, 
			// if applicable, or from the ORIGINAL DECIDED TO ADMIT DATE.
			// Go to the last TCIOutcome (Current outcome?), if it is DNA’d or Patient Cancelled, the count is only from this date.
			// ????????????????????
			Integer countSuspendedDays;
			DateTime dnaCancelledWorkDate = null;
			DateTime patientCancelledWorkDate = null;
			countSuspendedDays = 0;

			if(domPel.getElectiveListStatus() != null
					&& domPel.getElectiveListStatus().getElectiveListStatus() != null
					&& WaitingListStatus.SUSPENDED.equals(domPel.getElectiveListStatus().getElectiveListStatus())) 
			{
				if(domPel.getTCIDetails() != null
						&& domPel.getTCIDetails().getCurrentOutcome() != null
						&& domPel.getTCIDetails().getCurrentOutcome() != null
						&& domPel.getTCIDetails().getCurrentOutcome().getOutcome() != null
						&& AdmissionOfferOutcome.ADMISSION_CANCELLED_BY_PATIENT_2.equals(domPel.getTCIDetails().getCurrentOutcome().getOutcome())
						&& domPel.getTCIDetails().getCurrentOutcome().getStatusDateTime() != null)
				{
					dnaCancelledWorkDate = new DateTime(domPel.getTCIDetails().getCurrentOutcome().getStatusDateTime());
					countSuspendedDays= DateTime.daysDiff(new DateTime(), dnaCancelledWorkDate);
				}
				...
				if(domPel.getTCIDetails() != null
						...
						&& domPel.getTCIDetails().getCurrentOutcome() != null
						&& domPel.getTCIDetails().getCurrentOutcome().getOutcome() != null
						&& AdmissionOfferOutcome.PATIENT_FAILED_TO_ARRIVE_4.equals(domPel.getTCIDetails().getCurrentOutcome().getOutcome())
						&& domPel.getTCIDetails().getCurrentOutcome().getStatusDateTime() != null)
				{
					patientCancelledWorkDate = new DateTime(domPel.getTCIDetails().getCurrentOutcome().getStatusDateTime());
					countSuspendedDays= DateTime.daysDiff(new DateTime(), patientCancelledWorkDate);
				}

				if (dnaCancelledWorkDate == null && patientCancelledWorkDate == null)
				{
					if (domPel.getOriginalDecisionDate() != null)
					{
						DateTime workDate = new DateTime(domPel.getOriginalDecisionDate());
						countSuspendedDays= DateTime.daysDiff(new DateTime(), workDate);
					}
					else if (domPel.getDateOnList() != null)
					{
						DateTime workDate = new DateTime(domPel.getDateOnList());
						countSuspendedDays= DateTime.daysDiff(new DateTime(), workDate);						
					}
				}
			}
			cds.setCountSuspendedDays(countSuspendedDays);
			
			//ELEStatus
			// If PatientElectiveList status is Created, RequiresTCI, TCIGiven or Admitted - Output 01
			// If PatientElectiveList status is Removed or Suspended - Output 02
			if(domPel.getElectiveListStatus() != null
					&& domPel.getElectiveListStatus().getElectiveListStatus() != null)
			{
				LookupInstance status = domPel.getElectiveListStatus().getElectiveListStatus();
				if(WaitingListStatus.CREATED.getId() == status.getId()
						|| WaitingListStatus.REQUIRES_TCI.getId() == status.getId()
						|| WaitingListStatus.TCI_GIVEN.getId() == status.getId()
						|| WaitingListStatus.ADMITTED.getId() == status.getId())
				{
					cds.setELEStatus(formatStr("01", CDSElectiveListVo.getELEStatusMaxLength()));
				}
				else if(WaitingListStatus.REMOVED.getId() == status.getId()
						|| WaitingListStatus.SUSPENDED.getId() == status.getId())
				{
					cds.setELEStatus(formatStr("02", CDSElectiveListVo.getELEStatusMaxLength()));					
				}
			}
			else
			{
				cds.setELEStatus(formatStr(" ", CDSElectiveListVo.getELEStatusMaxLength()));					
			}

			//ELEType
			if(domPel.getElectiveAdmissionType() != null)
			{
				LookupInstance admType = domPel.getElectiveAdmissionType();
				if(ElectiveAdmissionType.ELECTIVE_TYPE11.getId() == admType.getId())
				{
					cds.setELEType(formatStr("11", CDSElectiveListVo.getELETypeMaxLength()));
				}
				else if(ElectiveAdmissionType.BOOKED_TYPE12.getId() == admType.getId())
				{
					cds.setELEType(formatStr("12", CDSElectiveListVo.getELETypeMaxLength()));
				}
			}
			else
			{
				cds.setELEType(formatStr(" ", CDSElectiveListVo.getELETypeMaxLength()));
			}
			
			//IntendedManagement
			if(domPel.getIntendedManagement() != null)
			{
				LookupInstance intendedMgmnt = domPel.getIntendedManagement();
				if(ManagementIntention.OVERNIGHT.getId() == intendedMgmnt.getId())
				{
					cds.setIntendedManagement(formatStr("1", CDSElectiveListVo.getIntendedManagementMaxLength()));
				}
				else if(ManagementIntention.DAY_CASE.getId() == intendedMgmnt.getId())
				{
					cds.setIntendedManagement(formatStr("2", CDSElectiveListVo.getIntendedManagementMaxLength()));
				}
			}
			else
			{ //Unknown
				cds.setIntendedManagement(formatStr(" ", CDSElectiveListVo.getIntendedManagementMaxLength()));
			}
			
			//IntendedProcedure
			if(domPel.getPrimaryProcedure() != null)
			{ // Operative procedure intended
				cds.setIntendedProcedure(formatStr("1", CDSElectiveListVo.getIntendedProcedureMaxLength()));
			}
			else if(domPel.getProcedureDescription() != null)
			{ // No operative procedure intended
				cds.setIntendedProcedure(formatStr("8", CDSElectiveListVo.getIntendedProcedureMaxLength()));
			}
			else
			{ // Unknown
				cds.setIntendedProcedure(formatStr("9", CDSElectiveListVo.getIntendedProcedureMaxLength()));
			}
			...
			// 2	Urgent
			// 3	Two Week Wait
			if(domPel.getPriority() != null)
			{
				cds.setPriorityTypeCode(formatStr(getExternalMapping(domPel.getPriority(), "CDS"), CDSElectiveListVo.getPriorityTypeCodeMaxLength()));
			}
			else
			{ //Unknown
				cds.setPriorityTypeCode(formatStr(" ", CDSElectiveListVo.getPriorityTypeCodeMaxLength()));
			}
			
			//DecidedToAdmitDate
			// If PatientElectiveList.CreatingAppointment  is not null use it’s AppointmentDate
			// Or if PatientElectiveList.OriginalDecisionDate is not null use it
			// Otherwise PatientElectiveList.DateOnList
			if(domPel.getCreatingAppointment() != null
					&& domPel.getCreatingAppointment().getAppointmentDate() != null)
			{
				cds.setDecidedToAdmitDate(domPel.getCreatingAppointment().getAppointmentDate());
			}
			else if(domPel.getOriginalDecisionDate() != null)
			{
				cds.setDecidedToAdmitDate(domPel.getOriginalDecisionDate());
			}
			else if(domPel.getDateOnList() != null)
			{
				cds.setDecidedToAdmitDate(domPel.getDateOnList());
			}
			
			//AgeAtCDSActivity
			cds.setAgeAtCDSActivity(999);
			cds.setAgeAtCDSActivity(calculateAge(cds.getDecidedToAdmitDate(), domPatient.getDob()));
			
			//OverseasVisitorStatus ?????????????????????????????????
			//			1	Check Referral for Patient Category
			//			Exempt from payment - subject to Reciprocal Healthcare Agreement
			//			2	Exempt from payment - other
			//			3	To pay hotel fees only
			//			4	To pay all fees
			//			8 - Not applicable (not an Overseas Visitor)
			//			9 - Charging rate not known
			cds.setOverseasVisitorStatus(formatStr(" ", CDSElectiveListVo.getOverseasVisitorStatusMaxLength()));
			
			//GuarenteedAdmissionDate (not used)
			cds.setGuarenteedAdmissionDate(null);
			
			//LastDNACancelledDate
			// Need to iterate through TCIDetails.CurrentOutcome and TCIDetails.OutcomeHistory
			//                     AND TCIHistory.CurrentOutcome and TCIHistory.OutcomeHistory
			// -- For each iteration, check TCIForPatientElectList.<outcome>.Outcome (AdmissionOfferOutcome) and check PATIENT_FAILED_TO_ARRIVE
			// -- and whether date is greater (after) working date dnaCancelledDate
			
			java.util.Date dnaCancelledDate = new java.util.Date();
			dnaCancelledDate = null;
			...
			if(domPel.getTCIDetails() != null
					...
					&& AdmissionOfferOutcome.PATIENT_FAILED_TO_ARRIVE_4.equals(domPel.getTCIDetails().getCurrentOutcome().getOutcome()))
			{
				dnaCancelledDate = domPel.getTCIDetails().getCurrentOutcome().getStatusDateTime();
			}
			
			if(domPel.getTCIDetails() != null
					&& domPel.getTCIDetails().getOutcomeHistory() != null)
			{
				List outcomeHistory = domPel.getTCIDetails().getOutcomeHistory();
				for(int x = 0; x < outcomeHistory.size(); x++)
				{
					TCIOutcomeForPatientElectiveList tciOutComeForPatientElectList = (TCIOutcomeForPatientElectiveList) outcomeHistory.get(x);
					if(AdmissionOfferOutcome.PATIENT_FAILED_TO_ARRIVE_4.equals(tciOutComeForPatientElectList.getOutcome())
						&& tciOutComeForPatientElectList.getStatusDateTime().after(dnaCancelledDate))
					{
						dnaCancelledDate = tciOutComeForPatientElectList.getStatusDateTime();
					}
				}
			}
			
			if(domPel.getTCIHistory() != null)
			{
				List tciForPatientElectiveList = domPel.getTCIHistory();
				for(int x = 0; x < tciForPatientElectiveList.size(); x++)
				{
					TCIForPatientElectiveList tciForPEL = (TCIForPatientElectiveList) tciForPatientElectiveList.get(x);
					if(tciForPEL.getCurrentOutcome() != null
							&& tciForPEL.getCurrentOutcome().getOutcome() != null
							&& tciForPEL.getCurrentOutcome().getStatusDateTime() != null
							&& AdmissionOfferOutcome.PATIENT_FAILED_TO_ARRIVE_4.equals(tciForPEL.getCurrentOutcome().getOutcome())
							&& tciForPEL.getCurrentOutcome().getStatusDateTime().after(dnaCancelledDate))
					{
						dnaCancelledDate = tciForPEL.getCurrentOutcome().getStatusDateTime();
					}
						
					List outcomeHistoryList = tciForPEL.getOutcomeHistory();
					for(int y = 0; y < outcomeHistoryList.size(); y++)
					{
						TCIOutcomeForPatientElectiveList tciOutCome = (TCIOutcomeForPatientElectiveList) outcomeHistoryList.get(y);
						if(tciOutCome.getOutcome() != null
								&& AdmissionOfferOutcome.PATIENT_FAILED_TO_ARRIVE_4.equals(tciOutCome.getOutcome())
								&& tciOutCome.getStatusDateTime() != null
								&& tciOutCome.getStatusDateTime().after(dnaCancelledDate))
						{
							dnaCancelledDate = tciOutCome.getStatusDateTime();
						}
					}
				}	
			}
			
			if(dnaCancelledDate != null)
			{
				cds.setLastDNACancelledDate(dnaCancelledDate);
			}
			
			
			//LastReviewedDate
			// Get the most recent record from the collection of PatientElectiveList.Reviews and use ReviewedDate
			if(domPel.getReviews() != null)
			{
				Date reviewDate = null;
				for (Object reviewPatElectListObject : domPel.getReviews())
				{
					ReviewPatientElectiveList review = (ReviewPatientElectiveList)reviewPatElectListObject;
					if(review.getReviewedDate().after(reviewDate))
					{
						cds.setLastReviewedDate(review.getReviewedDate());
					}
				}
			}
			...
			//ConsultantCode
			// National Consultant Code mapping value for PatientElectiveList.Consultant
			String consultantCode = null;
			if (domPel.getConsultant() != null)
			{
				consultantCode = getNatConsultantCode(domPel.getConsultant());
				if(consultantCode != null)
				{
					cds.setConsultantCode(formatStr(consultantCode, CDSElectiveListVo.getConsultantCodeMaxLength()));
				}
				else
					cds.setConsultantCode(formatStr(" ", CDSElectiveListVo.getConsultantCodeMaxLength()));
			}
			else
			{
				cds.setConsultantCode(formatStr(" ", CDSElectiveListVo.getConsultantCodeMaxLength()));
			...
			//SpecialtyCode
			// National Specialty Code associated with PatientElectiveList Service - Service
			String mainSpecCode = null;
			if(domPel.getElectiveList() != null
					&& domPel.getElectiveList().getService() != null
					&& domPel.getElectiveList().getService().getSpecialty() != null)
			{
				mainSpecCode = getExternalMapping(domPel.getElectiveList().getService().getSpecialty(), TaxonomyType.NAT_SPEC_CODE);
				if(mainSpecCode != null)
				{
					cds.setSpecialtyCode(formatStr(mainSpecCode, CDSElectiveListVo.getSpecialtyCodeMaxLength()));
				}
				else
					cds.setSpecialtyCode(formatStr(" ", CDSElectiveListVo.getSpecialtyCodeMaxLength()));
			}
			else
				cds.setSpecialtyCode(formatStr(" ", CDSElectiveListVo.getSpecialtyCodeMaxLength()));
			
			//TreatmentFuncCode
			// National Specialty Code associated with PatientElectiveList Service 
			cds.setTreatmentFuncCode(formatStr(cds.getSpecialtyCode(), CDSElectiveListVo.getTreatmentFuncCodeMaxLength()));
			
			//LocalSubSpecialtyCode
			// blank
			cds.setLocalSubSpecialtyCode(formatStr(" ", CDSElectiveListVo.getLocalSubSpecialtyCodeMaxLength()));
			
			
			// DATA GROUP: INTENDED PROCEDURE - CLINICAL ACTIVITY GROUP (OPCS)
			//ProcedureScheme
			cds.setProcedureScheme(formatStr("OPCS4", CDSElectiveListVo.getProcedureSchemeMaxLength()));
			
			//Procedure
			//OPCS4 mapping for PatientElectiveList.TCIDetails.Appointment.TheatreBooking.Procedure
			cds.setProcedure(formatStr(" ", CDSElectiveListVo.getProcedureMaxLength()));
			if(domPel.getTCIDetails() != null
					&& domPel.getTCIDetails().getAppointment() != null
					&& domPel.getTCIDetails().getAppointment().getTheatreBooking() != null
					&& domPel.getTCIDetails().getAppointment().getTheatreBooking().getProcedure() != null
					&& domPel.getTCIDetails().getAppointment().getTheatreBooking().getProcedure().getTaxonomyMap() != null)
			{
				List mapList = domPel.getTCIDetails().getAppointment().getTheatreBooking().getProcedure().getTaxonomyMap();
				for(int y = 0; y < mapList.size(); y++)
				{
					TaxonomyMap taxMap = (TaxonomyMap) mapList.get(y);
					if(TaxonomyType.OPCS4.equals(taxMap.getTaxonomyCode()))
					{
						cds.setProcedure(formatStr(taxMap.getTaxonomyCode(), CDSElectiveListVo.getProcedureMaxLength()));
					}
				}
			}
			
			//ProcedureDate
			// PatientElectiveList.TCIDetails.Appointment.AppointmentDate
			if(domPel.getTCIDetails() != null
					&& domPel.getTCIDetails().getAppointment() != null
					&& domPel.getTCIDetails().getAppointment().getAppointmentDate() != null)
			{
				cds.setProcedureDate(domPel.getTCIDetails().getAppointment().getAppointmentDate());
			}
			
			//ProfRegistrationCode
			// If PatientElectiveList.Consultant has National Consultant Code, output 03 (General Medical Council), otherwise output 08 (Health and Care Professional Council)
			cds.setProfRegistrationCode(formatStr("08", CDSElectiveListVo.getProfRegistrationCodeMaxLength()));
			if (domPel.getConsultant() != null)
			{
				consultantCode = getNatConsultantCode(domPel.getConsultant());
				if(consultantCode != null)
				{
					cds.setProfRegistrationCode(formatStr("03", CDSElectiveListVo.getProfRegistrationCodeMaxLength()));
				}
			}
			
			//ProfRegistrationID
			// GMC number (NAT_GP_CODE?) for listowner i.e. PatientElectiveList.TCIDetails.Appointment.SessionSlot.SlotResp.Hcp.Mos
			cds.setProfRegistrationID(formatStr(" ", CDSElectiveListVo.getProfRegistrationIDMaxLength()));
			if(domPel.getTCIDetails() != null
					&& domPel.getTCIDetails().getAppointment() != null
					&& domPel.getTCIDetails().getAppointment().getSessionSlot() != null
					&& domPel.getTCIDetails().getAppointment().getSessionSlot().getSlotResp() != null
					&& domPel.getTCIDetails().getAppointment().getSessionSlot().getSlotResp().getHcp() != null
					&& domPel.getTCIDetails().getAppointment().getSessionSlot().getSlotResp().getHcp().getMos() != null
					&& domPel.getTCIDetails().getAppointment().getSessionSlot().getSlotResp().getHcp().getMos().getCodeMappings() != null
					&& domPel.getTCIDetails().getAppointment().getSessionSlot().getSlotResp().getHcp().getMos().getCodeMappings().size() > 0)
			{
				for (int y = 0; y<domPel.getTCIDetails().getAppointment().getSessionSlot().getSlotResp().getHcp().getMos().getCodeMappings().size(); y++)
				{
					TaxonomyMap map = (TaxonomyMap) domPel.getTCIDetails().getAppointment().getSessionSlot().getSlotResp().getHcp().getMos().getCodeMappings().get(y);
					if(map.getId() == TaxonomyType.NAT_GP_CODE.getId())
					{
						cds.setProfRegistrationID(formatStr(map.getTaxonomyCode(), CDSElectiveListVo.getProfRegistrationIDMaxLength()));
					}
				}
			}
			...
			CDSWaitingListLocGroup cdsWaitingListLocGroup = new CDSWaitingListLocGroup();

			//LocationClass
			cdsWaitingListLocGroup.setLocationClass(formatStr("01", CDSWaitingListLocGroupVo.getLocationClassMaxLength()));

			//SiteCodeTreatment
			// National Location Code for PatientElectiveList.CatsReferral.ReferralDetails.Location
			cdsWaitingListLocGroup.setSiteCodeTreatment(formatStr(" ", CDSWaitingListLocGroupVo.getSiteCodeTreatmentMaxLength()));
			if(domPel.getReferral() != null
					&& domPel.getReferral().getReferralDetails() != null
					&& domPel.getReferral().getReferralDetails().getLocation() != null
					&& domPel.getReferral().getReferralDetails().getLocation().getCodeMappings() != null
					&& domPel.getReferral().getReferralDetails().getLocation().getCodeMappings().size() > 0)
			{
				List codeMap = domPel.getReferral().getReferralDetails().getLocation().getCodeMappings();
				for (int y = 0; y<codeMap.size(); y++)
				{
					TaxonomyMap taxMap = (TaxonomyMap) codeMap.get(y);
					if(TaxonomyType.NAT_LOC_CODE.equals(taxMap.getTaxonomyName()))
					{
						cdsWaitingListLocGroup.setSiteCodeTreatment(formatStr(taxMap.getTaxonomyCode(), CDSWaitingListLocGroupVo.getSiteCodeTreatmentMaxLength()));
					}
				}
			}
			...
			//PatientGPCode
			cds.setPatientGPCode(formatStr(" ", CDSElectiveListVo.getPatientGPCodeMaxLength()));
			if(domPel.getPatient() != null
					&& domPel.getPatient().getGp() != null)
			{
				cds.setPatientGPCode(formatStr(getGPCode(domPel.getPatient().getGp()), CDSElectiveListVo.getPatientGPCodeMaxLength()));
			}

			//PatientGPPractice
			cds.setPatientGPPractice(formatStr(" ", CDSElectiveListVo.getPatientGPPracticeMaxLength()));
			if(domPel.getPatient() != null
					&& domPel.getPatient().getPractice() != null)
			{
				cds.setPatientGPPractice(formatStr(getPracticeCode(domPel.getPatient().getPractice()), CDSElectiveListVo.getPatientGPPracticeMaxLength()));
			}
			else
			{
				if(domPel.getPatient() != null
						&& domPel.getPatient().getGpSurgery() != null
						&& domPel.getPatient().getGpSurgery().getParentLocation() != null)
				{
					cds.setPatientGPPractice(formatStr(getPracticeCode((LocSite) domPel.getPatient().getGpSurgery().getParentLocation()), CDSElectiveListVo.getPatientGPPracticeMaxLength()));
				}
			}

			//ReferrerCode
			//	If PatientElectiveList.Referral.ReferralLetterDetails.RefererType is:
			//	 	GP – Output GMC number for PatientElectiveList.Referral.ReferralLetterDetails.GPName
			//					OR
			//	  	Consultant – Output National Consultant Code for PatientElectiveList.Referral.ReferralLetterDetails.ReferringConsultant
			//					OR
			//	  	Self – Output X9999998
			//					OR
			//		AHP - PatientElectiveList.Referral.ReferralLetterDetails.AlliedHealthProfName
			//	Otherwise
			//				Output R9999981
			
			cds.setReferrerCode(formatStr("R9999981", CDSElectiveListVo.getReferrerCodeMaxLength()));
			if(domPel.getReferral() != null
					&& domPel.getReferral().getReferralDetails() != null
					&& domPel.getReferral().getReferralDetails().getReferrerType() != null)
			{
				LookupInstance refType = domPel.getReferral().getReferralDetails().getReferrerType();
				if(SourceOfReferral.CONSULTANT.getId() == refType.getId())
				{
					if(domPel.getReferral().getReferralDetails().getConsultant() != null)
					{
						consultantCode = getNatConsultantCode(domPel.getReferral().getReferralDetails().getConsultant());
						if(consultantCode != null)
						{
							cds.setReferrerCode(formatStr(consultantCode, CDSElectiveListVo.getReferrerCodeMaxLength()));
						}
					}
				}
				else if(SourceOfReferral.GP.getId() == refType.getId())
				{
					if(domPel.getReferral().getReferralDetails().getGPName() != null)
					{
						String referrerCode = getGPCode(domPel.getReferral().getReferralDetails().getGPName());
						if(referrerCode != null)
						{
							cds.setReferrerCode(formatStr(referrerCode, CDSElectiveListVo.getReferrerCodeMaxLength()));
						}
					}
				}
				else if(SourceOfReferral.SELF.getId() == refType.getId())
				{
					cds.setReferrerCode(formatStr("X9999998", CDSElectiveListVo.getReferrerCodeMaxLength()));
				}
				else if(SourceOfReferral.AHP.getId() == refType.getId())
				{
					if(domPel.getReferral().getReferralDetails().getAlliedHealthProfName() != null)
					{
						cds.setReferrerCode(formatStr(domPel.getReferral().getReferralDetails().getAlliedHealthProfName(), CDSElectiveListVo.getReferrerCodeMaxLength()));
					}
				}
			}
			...
			//ReferrerPractice
			//	If PatientElectiveList.Referral.ReferralLetterDetails.RefererType is:
			//	   GP – Output Practice Code for PatientElectiveList.Referral.ReferralLetterDetails.Practice
			cds.setReferrerPractice(formatStr(" ", CDSElectiveListVo.getReferrerPracticeMaxLength()));
			if(domPel.getReferral() != null
					&& domPel.getReferral().getReferralDetails() != null
					&& domPel.getReferral().getReferralDetails().getReferrerType() != null
					&& SourceOfReferral.GP.equals(domPel.getReferral().getReferralDetails().getReferrerType())
					&& domPel.getReferral().getReferralDetails().getPractice() != null)
			{
				cds.setPatientGPPractice(formatStr(getPracticeCode(domPel.getReferral().getReferralDetails().getPractice()), CDSElectiveListVo.getReferrerPracticeMaxLength()));			
			}
			

			// DATA GROUP: OFFER OF ADMISSION
			//OutcomeCode
			// External CDS mapping for PatientElectiveList.TCIDetails.CurrentOutcome.AdmissionOfferOutcome
			cds.setOutcomeCode(formatStr(" ", CDSElectiveListVo.getOutcomeCodeMaxLength()));
			if(domPel.getTCIDetails() != null
					&& domPel.getTCIDetails().getCurrentOutcome() != null
					&& domPel.getTCIDetails().getCurrentOutcome().getOutcome() != null)
			{
				cds.setOutcomeCode(formatStr(getExternalMapping(domPel.getTCIDetails().getCurrentOutcome().getOutcome()), CDSElectiveListVo.getOutcomeCodeMaxLength()));
			}

			//DateTCIOffered
			if(domPel.getTCIDetails() != null
					&& domPel.getTCIDetails().getDateTCIOffered() != null)
			{
				cds.setDateTCIOffered(domPel.getTCIDetails().getDateTCIOffered());
			}
			
			//ERODate
			if(domPel.getEROD() != null
					&& domPel.getEROD().getERODDate1() != null)
			{
				cds.setERODate(domPel.getEROD().getERODDate1());
			}
			...
			if(domPel.getCreatingAppointment() != null
					&& domPel.getCreatingAppointment().getAppointmentDate() != null)
			{
				cds.setOrigDecisionToAdmitDate(domPel.getCreatingAppointment().getAppointmentDate());
			}
			else if(domPel.getOriginalDecisionDate() != null)
			{
				cds.setOrigDecisionToAdmitDate(domPel.getOriginalDecisionDate());
			}
			else if(domPel.getDateOnList() != null)
			{
				cds.setOrigDecisionToAdmitDate(domPel.getDateOnList());
			}

			//RemovalReason
			// If PatientElectiveList.ElectiveListStatus.ElectiveListStatus = REMOVED 
			//		Output	1 if RemovalReason = Patient Admitted Electively
			//				2 if RemovalReason = Admitted Emergency for same condition
			//				3 if Removal Reason is Patient Died
			//				Otherwise output 4
			
			if(domPel.getElectiveListStatus() != null
					&& domPel.getElectiveListStatus().getElectiveListStatus() != null
					&& WaitingListStatus.REMOVED.equals(domPel.getElectiveListStatus().getElectiveListStatus()))
			{
				if(domPel.getElectiveListStatus().getRemovalReason() != null)
				{
					LookupInstance reason = domPel.getElectiveListStatus().getRemovalReason();
					if(ElectiveListStatusReason.PATIENT_ADMITTED_ELECTIVELY.getId() == reason.getId())
					{
						cds.setRemovalReason(formatStr("1", CDSElectiveListVo.getRemovalReasonMaxLength()));
					}
					else if(ElectiveListStatusReason.PATIENT_ADMITTED_AS_AN_EMERGENCY_FOR_THE_SAME_CONDITION.getId() == reason.getId())
					{
						cds.setRemovalReason(formatStr("2", CDSElectiveListVo.getRemovalReasonMaxLength()));
					}
					else if(ElectiveListStatusReason.PATIENT_DIED.getId() == reason.getId())
					{
						cds.setRemovalReason(formatStr("3", CDSElectiveListVo.getRemovalReasonMaxLength()));
					}
				}
				else
				{
					cds.setRemovalReason(formatStr("4", CDSElectiveListVo.getRemovalReasonMaxLength()));
				}
			}
			else
			{
				cds.setRemovalReason(formatStr(" ", CDSElectiveListVo.getRemovalReasonMaxLength()));
			}
			...
			if(domPel.getElectiveListStatus() != null
					&& domPel.getElectiveListStatus().getElectiveListStatus() != null
					&& WaitingListStatus.REMOVED.equals(domPel.getElectiveListStatus().getElectiveListStatus())
					&& domPel.getElectiveListStatus().getStatusDateTime() != null)
			{
				cds.setRemovalDate(domPel.getElectiveListStatus().getStatusDateTime());
			}
			
			
			electiveListMap.put(String.valueOf(domPel.getId()), cds);
		...
		// Now work through the list to save items
		for (String cds: electiveListMap.keySet())
		{
			try
			{
				factory.saveNoFlush((DomainObject) electiveListMap.get(cds));
				insertCounter++;
				if (insertCounter %100 == 0)
				{
					transaction.commit();
...
//		transaction.commit();
	...
	*/
	...
	public Integer generateInpatCDSData(ims.framework.utils.Date fromDate, ims.framework.utils.Date toDate) throws DomainInterfaceException, StaleObjectException 
	{
		...
		// WDEV-13368
		String deleteOutpatientSql = " delete from core_cdsprocdetail where core_cdsinpatient_opcsproced in (select id from core_cdsinpatient where core_cdsinpatient.startdate between ? and ?)";
		deleteFromTable(deleteOutpatientSql, fromDate, toDate);
		
		deleteOutpatientSql = " delete from core_cdsdiagdetail where core_cdsinpatient_icddiagnos in (select id from core_cdsinpatient where core_cdsinpatient.startdate between ? and ?)";
		deleteFromTable(deleteOutpatientSql, fromDate, toDate);
		...
		{
			hql.append(" and pe.eventDateTime < :toDate"); //WDEV-19260
			markers.add("toDate");
		...
		for (int i=0; i<lst.size(); i++)
		{
			...
			IntraOperativeCareRecord domIntraCare = getIntraCareRecordFromRefCode(factory, domRefCode, domAdmission.getAdmissionDateTime(), domDischarge.getDischargeDateTime());
			...
			{
				if (domDischarge != null && domDischarge.getDischargeDateTime() != null) 
					cds.setRTTEndDate(domDischarge.getDischargeDateTime());  // wdev-9720  discharge date from discharge record
			}
			...
			if (domDischarge.getDischargeDateTime() != null)
				cds.setPatientClassification(formatStr(getPatientClass(domAdmission.getAdmissionDateTime(), domDischarge.getDischargeDateTime()), CDSInpatientVo.getPatientClassificationMaxLength()));  
			else
			...
			if (domDischarge != null)
			{
				cds.setDischargeDate(domDischarge.getDischargeDateTime());
				...
				cds.setEpisodeEndDate(domDischarge.getDischargeDateTime());
			}
			...
			if (firstProc != null)
			{
				...
				if (domIntra == null)
					domIntra = getIntraFromProcedure(factory, firstProc, domAdmission.getAdmissionDateTime(), domDischarge.getDischargeDateTime());
	...
	public CDSResultVo buildCDSReport(ims.framework.utils.Date fromDate, ims.framework.utils.Date toDate, String nlc, Integer imsReportId, String sessionId, String requestUrl) throws DomainInterfaceException, StaleObjectException
	{
		...
		//WDEV-18912
		else if(ELECTIVELIST_REPORT_ID == imsReportId)
			histVo.setReportType(CDSReportType.ELECTIVELIST); //WDEV-18912
		...
		//WDEV-18912
		else if (imsReportId == ELECTIVELIST_REPORT_ID)
			info = ELECTIVELIST_REPORT; //WDEV-18912
	    ...
	    while ((strLine = br.readLine()) != null)
	    {
	        ...
	        if (strLine != null && strLine.trim().length() > 0)
	        {
	    		...
	    		if(maxLine.getRecords() < MAX_RECORDS_LOG)
	    		{
	        		...
	        		sbLog.append(strLine.replaceAll(" ", "&nbsp;").replaceAll("-", "&minus;"));
