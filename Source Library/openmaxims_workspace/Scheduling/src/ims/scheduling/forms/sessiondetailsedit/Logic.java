...
import ims.scheduling.forms.sessiondetailsedit.GenForm.lyrSessEditLayer.tabActivitiesWithSlotsContainer.grdActivitiesSlotsRow;
import ims.scheduling.forms.sessiondetailsedit.GenForm.lyrSessEditLayer.tabActivitiesWithSlotsContainer.grdSlotsFlexibleRow;
...
public class Logic extends BaseLogic
{
	...
	private static final String LIST_OWNER = "List Owner";
	...
	private void initialise()
	{
		...
		//WDEV-19518
		form.lyrSessEdit().tabGeneral().lblLocStar().setTextColor(Color.Red);
		form.lyrSessEdit().tabGeneral().lblLocStar().setTooltip("Required");
		form.lyrSessEdit().tabGeneral().lblCaseNoteLocStar().setTextColor(Color.Red);
		form.lyrSessEdit().tabGeneral().lblCaseNoteLocStar().setTooltip("Required");
		
		form.lyrSessEdit().tabListOwners().ccConsultant().initialize(MosType.HCP);	//wdev-19419
	}
	
	private void updateControlState() //WDEV-21137
	{
		...
		form.getContextMenus().Scheduling.hideAllSessionDetailsEditFlexibleSlotsActivityMenuItems();
		form.getContextMenus().Scheduling.hideAllSessionDetailsEditFlexibleSlotMenuItems();
		
		boolean isTheatre = false;
		boolean isFixed = true;
		if(form.getLocalContext().getSessionIsNotNull())
		{
			isTheatre = SchProfileType.THEATRE.equals(form.getLocalContext().getSession().getSessionProfileType());
		...
		form.lyrSessEdit().tabTheatreProcedures().setHeaderVisible(false); //WDEV-22949;
		
		//WDEV-18453 
		form.lyrSessEdit().tabGeneral().cmbConsMedia().setEnabled(FormMode.EDIT.equals(form.getMode()) && form.getLocalContext().getSession() != null && SchProfileType.OUTPATIENT.equals(form.getLocalContext().getSession().getSessionProfileType()));
		//WDEV-12918
		...
		//WDEV-19518 
		form.lyrSessEdit().tabGeneral().lblCaseNoteLocStar().setVisible(FormMode.EDIT.equals(form.getMode()) && !form.lyrSessEdit().tabGeneral().chkCaseNotesNotRequired().getValue() && !isTheatre);//WDEV-22266
		form.lyrSessEdit().tabGeneral().chkCaseNotesNotRequired().setEnabled(FormMode.EDIT.equals(form.getMode()) && !isTheatre);////WDEV-22266
		form.lyrSessEdit().tabGeneral().lblCaseNoteLoc().setVisible(!form.lyrSessEdit().tabGeneral().chkCaseNotesNotRequired().getValue());
		form.lyrSessEdit().tabGeneral().txtCaseNoteLocation().setVisible(!form.lyrSessEdit().tabGeneral().chkCaseNotesNotRequired().getValue());
		form.lyrSessEdit().tabGeneral().txtCaseNoteLocation().setEnabled(false);
		form.lyrSessEdit().tabGeneral().btnCaseNoteLocation().setVisible(!form.lyrSessEdit().tabGeneral().chkCaseNotesNotRequired().getValue());
		form.lyrSessEdit().tabGeneral().btnCaseNoteLocation().setEnabled(FormMode.EDIT.equals(form.getMode()) && !form.lyrSessEdit().tabGeneral().chkCaseNotesNotRequired().getValue()&& !isTheatre);//WDEV-22266
		// end WDEV-19518 
		//WDEV-8973 - activities for flexible profile
		form.lyrSessEdit().tabActivity().setHeaderVisible(false);
		form.lyrSessEdit().tabActivitiesWithSlots().setHeaderVisible(false);
		...
		if(!isFixed)
		{
			...
			form.lyrSessEdit().tabActivity().setHeaderVisible(ConfigFlag.DOM.PROFILE_FLEXIBLE_TYPE.getValue().equals("WITHOUT_SLOTS"));
			form.lyrSessEdit().tabActivitiesWithSlots().setHeaderVisible(ConfigFlag.DOM.PROFILE_FLEXIBLE_TYPE.getValue().equals("WITH_SLOTS"));
		}
		...
		else if (form.getMode().equals(FormMode.EDIT))
		{
			...
			form.getContextMenus().Scheduling.getSessionDetailsListOwnersEDITItem().setVisible(form.lyrSessEdit().tabListOwners().grdListOwner().getSelectedRow() != null && form.lyrSessEdit().tabListOwners().grdListOwner().getSelectedRow().isReadOnly() && (!form.getLocalContext().getHasOwnerProceduresIsNotNull() || !form.getLocalContext().getHasOwnerProcedures())); //WDEV-21773
			...
			boolean flexibleSlotOpen = form.lyrSessEdit().tabActivitiesWithSlots().grdSlotsFlexible().getValue() != null && Status_Reason.SLOTOPENED.equals(form.lyrSessEdit().tabActivitiesWithSlots().grdSlotsFlexible().getValue().getStatus()) && form.lyrSessEdit().tabActivitiesWithSlots().grdSlotsFlexible().getValue().getAppointment() == null;
			
			form.getContextMenus().Scheduling.getSessionDetailsEditFlexibleSlotADDItem().setVisible(true);
			form.getContextMenus().Scheduling.getSessionDetailsEditFlexibleSlotBLOCKItem().setVisible(flexibleSlotOpen);
			form.getContextMenus().Scheduling.getSessionDetailsEditFlexibleSlotCANCELItem().setVisible(flexibleSlotOpen);
			
			form.getContextMenus().Scheduling.getSessionDetailsEditFlexibleSlotsActivityADDItem().setVisible(true);
			form.getContextMenus().Scheduling.getSessionDetailsEditFlexibleSlotsActivityREMOVEItem().setVisible(form.lyrSessEdit().tabActivitiesWithSlots().grdActivitiesSlots().getValue() != null);
	...
	private void enableRowSelection()
	{
		...
		for (int i = 0; i < form.lyrSessEdit().tabActivitiesWithSlots().grdActivitiesSlots().getRows().size(); i++)
		{
			form.lyrSessEdit().tabActivitiesWithSlots().grdActivitiesSlots().getRows().get(i).setSelectable(true);
		}
		
		for (int i = 0; i < form.lyrSessEdit().tabActivitiesWithSlots().grdSlotsFlexible().getRows().size(); i++)
		{
			form.lyrSessEdit().tabActivitiesWithSlots().grdSlotsFlexible().getRows().get(i).setSelectable(true);
		}
	}
	...
	{	
		...
		SessionShortVoCollection voCollSession = domain.listSession(form.qmbProfile().getValue(), form.qmbDoService().getValue(), form.dteProfFrom().getValue(), form.dteProfTo().getValue(),form.cmbStatus().getValue());// 	WDEV-15190,wdev-19395
	...
	private void populateGrid(SessionShortVoCollection voCollSession)
	{
		...
		for (int i = 0; i < voCollSession.size(); i++)
		{
			...
			if( voCollSession.get(i).getSessionStatusIsNotNull())
				sRow.setColumnStatus(voCollSession.get(i).getSessionStatus().getIItemText());
			//------------
	...
	private void clearSearchCriteria()
	{
		...
		form.grdSessions().getRows().clear();
		form.cmbStatus().setValue(null);	//wdev-19395
	}
	...
	private void populateScreenFromData()
	{
		...
		boolean isFixedSession = voSession.getIsFixed();
		...
		//WDEV-19518 
		form.lyrSessEdit().tabGeneral().chkCaseNotesNotRequired().setValue(voSession.getCaseNoteFolderNotRequired());
		if (voSession.getCaseNoteFolderLocationIsNotNull())
			form.lyrSessEdit().tabGeneral().txtCaseNoteLocation().setValue(voSession.getCaseNoteFolderLocation().getName());
		...
		{
			TheatreDetailVoCollection voCollTheatreProcs = voSession.getTheatreProceduresRemaining().getTheatreDetails();
		...
		*/
		...
		if(voSession.getSessionActivitiesIsNotNull())
		{
			...
			{
				if (!isFixedSession && ConfigFlag.DOM.PROFILE_FLEXIBLE_TYPE.getValue().equals("WITH_SLOTS"))
				{
					for(SessionActivityLiteVo voSessionAct : voCollSessionAct)
					{
						addFlexibleActivityRow(voSessionAct);
					}
				}
				else
					for(SessionActivityLiteVo voSessionAct : voCollSessionAct)
		...
		// Booking Rights from Profile
		populateBookingRightsGrid(voSession); //WDEV-21137
		
		//wdev-19419
		form.lyrSessEdit().tabListOwners().cmbListType().setValue(voSession.getListType());
		form.lyrSessEdit().tabListOwners().ccConsultant().setValue(voSession.getResponsibleHCP());
		//-----------
		...
		if(voSession.getParentChildSlotsIsNotNull())
		{
			...
			if (voCollPCSlots != null)
			{
				...
				if (!isFixedSession && ConfigFlag.DOM.PROFILE_FLEXIBLE_TYPE.getValue().equals("WITH_SLOTS"))
				{
					...
					{
						addOutpatientFlexibleRow(voPcSlot);
					}
				}
				else
					for (SessionParentChildSlotVo voPcSlot : voCollPCSlots)
		...
		//WDEV-19208
		form.txtSessionComment().setValue(voSession.getSessionComment());
	}
	...
	private void addOutpatientFlexibleRow(SessionParentChildSlotVo voPcSlot)
	{
		if (voPcSlot == null)
			return;
		
		grdSlotsFlexibleRow row = form.lyrSessEdit().tabActivitiesWithSlots().grdSlotsFlexible().getRows().newRow();
		 
		row.setColStartTime(voPcSlot.getStartTime() != null ? voPcSlot.getStartTime().toString() : "");
		
		Time endTime = voPcSlot.getStartTime().copy();
		
		if (voPcSlot.getStartTime() != null && voPcSlot.getDuration() != null)
		{
			 endTime.addMinutes(voPcSlot.getDuration());
		}
		
		row.setColEndTime(voPcSlot.getStartTime() != null && voPcSlot.getDuration() != null ? endTime.toString() : "");
		row.setColBooked(voPcSlot.getAppointment() != null);
		
		row.setColSlotOwner(voPcSlot.getSlotResp() != null && voPcSlot.getSlotResp().getHcp() != null && voPcSlot.getSlotResp().getHcp().getName() != null ? voPcSlot.getSlotResp().getHcp().getName().toString() : "");
		row.setTooltipForColSlotOwner(row.getColSlotOwner());
		
		row.setColPriority(voPcSlot.getPriority());
		row.setColStatus(getImage(voPcSlot.getStatus()));
		row.setTooltipForColStatus(voPcSlot.getStatus() != null ? voPcSlot.getStatus().getText() : "");
		
		row.setColActivity("Open"); //booking in an Outpatient Flexible Slot is not yet coded, will have to be modified at that time.
		
		if (SchedulingPriority.URGENT.equals(voPcSlot.getPriority()))
		{
			row.setTextColor(Color.Red);
		}
		
		row.setValue(voPcSlot);
	...
	{
		//WDEV-19208
		form.txtSessionComment().setValue(null);
		...
		//WDEV-19518
		form.lyrSessEdit().tabGeneral().chkCaseNotesNotRequired().setValue(null);
		...
		//wdev-19419
		form.lyrSessEdit().tabListOwners().cmbListType().setValue(null);
		form.lyrSessEdit().tabListOwners().ccConsultant().setValue(null);
		...
		form.lyrSessEdit().tabActivitiesWithSlots().grdActivitiesSlots().getRows().clear();
		form.lyrSessEdit().tabActivitiesWithSlots().grdSlotsFlexible().getRows().clear();
	...
	private String[] getUIValidationRules(SessionVo voSession)
	{
		...
		if(voSession != null)
		{
			...
			Boolean isTheatre = form.getLocalContext().getSessionIsNotNull() && SchProfileType.THEATRE.equals(form.getLocalContext().getSession().getSessionProfileType());//WDEV-22266
			//WDEV-19518
			if (voSession.getCaseNoteFolderLocation() == null && !Boolean.TRUE.equals(voSession.getCaseNoteFolderNotRequired()) && !isTheatre)
				errors.add("General Details Tab : Case Note Folders Location is mandatory");
	...
	private SessionVo populateDataFromScreen(SessionVo voSession)
	{
		...
		{
			if (form.lyrSessEdit().tabSlotDetails().isHeaderVisible())
			{
				// Slot details
			...
			//WDEV-19497
			else if (form.lyrSessEdit().tabActivitiesWithSlots().isHeaderVisible())
			{
				SessionParentChildSlotVoCollection voCollSessionSlots = new SessionParentChildSlotVoCollection();
				
				// Outpatient Flexible Slots Details
				for (int i = 0; i < form.lyrSessEdit().tabActivitiesWithSlots().grdSlotsFlexible().getRows().size(); i++)
				{
					grdSlotsFlexibleRow row = form.lyrSessEdit().tabActivitiesWithSlots().grdSlotsFlexible().getRows().get(i);

					SessionParentChildSlotVo voSlot = row.getValue();

					voSlot.setPriority(row.getColPriority());
					voCollSessionSlots.add(voSlot);
				}

				voSession.setParentChildSlots(voCollSessionSlots);
			}
			...
			if (form.lyrSessEdit().tabActivity().isHeaderVisible())
			{
				for (int i = 0; i < form.lyrSessEdit().tabActivity().grdActivities().getRows().size(); i++)
				{
					GenForm.lyrSessEditLayer.tabActivityContainer.grdActivitiesRow acRow = form.lyrSessEdit().tabActivity().grdActivities().getRows().get(i);

					if (acRow.getValue() == null || acRow.getcolActivity().getValue() == null) // WDEV-18646
						continue;
			...
			else if (form.lyrSessEdit().tabActivitiesWithSlots().isHeaderVisible())
			{
				for (int i = 0; i < form.lyrSessEdit().tabActivitiesWithSlots().grdActivitiesSlots().getRows().size(); i++)
				{
					grdActivitiesSlotsRow row = form.lyrSessEdit().tabActivitiesWithSlots().grdActivitiesSlots().getRows().get(i);

					if (row.getValue() == null || row.getColActivity().getValue() == null)
						continue;

					SessionActivityLiteVo voSessActivity = row.getValue();
					voSessActivity.setActivity((ActivityLiteVo) row.getColActivity().getValue());
					voSessActivity.setIsActive(row.getColActive());
					voCollSessActivity.add(voSessActivity);
				}
			}
		...
		//wdev-19419
		voSession.setListType(form.lyrSessEdit().tabListOwners().cmbListType().getValue());
		voSession.setResponsibleHCP((HcpLiteVo) form.lyrSessEdit().tabListOwners().ccConsultant().getValue());
		...
		for (int i = 0; i < form.lyrSessEdit().tabListOwners().grdListOwner().getRows().size(); i++)
		{
			...
			//WDEV-21773
			voListOwner.setListOwner(gRow.getColListOwner());
			voListOwner.setAttendingClinician(gRow.getColAttendingClinician());
	...
	protected void onContextMenuItemClick(int menuItemID, Control sender) throws PresentationLogicException //WDEV-21137
	{
		switch (menuItemID)
		{
			case GenForm.ContextMenus.SchedulingNamespace.SessionDetailsEdit.EDIT :
				...
				if (form.getLocalContext().getSessionIsNotNull() && form.getLocalContext().getSession().getCaseNoteFolderLocation() != null && !Boolean.TRUE.equals(form.getLocalContext().getSession().getCaseNoteFolderNotRequired())) //WDEV-19631
				{
					if (!Boolean.TRUE.equals(form.getLocalContext().getSession().getCaseNoteFolderLocation().getCaseNoteFolderLocation()))
					{
						form.getLocalContext().getSession().setCaseNoteFolderLocation(null);
						form.lyrSessEdit().tabGeneral().txtCaseNoteLocation().setValue(null);
					}
			...
			case GenForm.ContextMenus.SchedulingNamespace.SessionDetailsEditSlots.BLOCK :
				form.getLocalContext().setCancelBlockCM(BLOCK_SLOT);
			...
			case GenForm.ContextMenus.SchedulingNamespace.SessionDetailsEditSlots.CANCEL :
				form.getLocalContext().setCancelBlockCM(CANCEL_SLOT);
				engine.open(form.getForms().Scheduling.ReasonTextDialog);
			break;
			...
			case GenForm.ContextMenus.SchedulingNamespace.SessionDetailsEditFlexibleSlot.ADD:
				form.getGlobalContext().Scheduling.setSelectedSessionListOwners(form.lyrSessEdit().tabListOwners().grdListOwner().getValues());
				engine.open(form.getForms().Scheduling.SessionFlexibleSlotDialog, new Object[] {form.getLocalContext().getSession()});
				break;
			case GenForm.ContextMenus.SchedulingNamespace.SessionDetailsEditFlexibleSlot.BLOCK:
				form.getLocalContext().setCancelBlockCM(BLOCK_SLOT);
				engine.open(form.getForms().Scheduling.ReasonTextDialog);
				break;
			case GenForm.ContextMenus.SchedulingNamespace.SessionDetailsEditFlexibleSlot.CANCEL:
				form.getLocalContext().setCancelBlockCM(CANCEL_SLOT);
				engine.open(form.getForms().Scheduling.ReasonTextDialog);
				break;
				
			case GenForm.ContextMenus.SchedulingNamespace.SessionDetailsEditFlexibleSlotsActivity.ADD:
				addFlexibleActivityRow(null);
				break;
			case GenForm.ContextMenus.SchedulingNamespace.SessionDetailsEditFlexibleSlotsActivity.REMOVE:
				form.lyrSessEdit().tabActivitiesWithSlots().grdActivitiesSlots().removeSelectedRow();
				break;
	...
	protected void onFormDialogClosed(FormName formName, DialogResult result) throws PresentationLogicException
	{
		if (formName.equals(form.getForms().Scheduling.SessionSlotDialog))
		{
			...
			{
				SessionVo voSession = form.getLocalContext().getSession();
				SessionSlotVoCollection sessionSlots = form.getGlobalContext().Scheduling.getSessionSlots();
				...
				{
					SessionSlotVo slot = sessionSlots.get(i);

					slot.setSession(form.getLocalContext().getSession());
				...
				{
					int remainingSlots = voSession.getRemainingSlots();
					remainingSlots += sessionSlots.size();
					voSession.setRemainingSlots(remainingSlots);

					int totalslots = voSession.getTotalSlots();
					totalslots += sessionSlots.size();
		...
		{
			if (form.getGlobalContext().Admin.getProfileLocationIsNotNull())
			{	
				setLocation();
			}
			else if (form.getGlobalContext().Admin.getProfileCaseNoteFoldersLocationIsNotNull())
			{
				setCaseNoteFolderLocation(form.getGlobalContext().Admin.getProfileCaseNoteFoldersLocation());
			}
		}
		
		if(formName.equals(form.getForms().Scheduling.ReasonTextDialog) && DialogResult.OK.equals(result))
		{
			
			if (form.getLocalContext().getCancelBlockCM() == BLOCK_SLOT)
			{
				if (form.lyrSessEdit().tabActivitiesWithSlots().grdSlotsFlexible().getSelectedRow() != null )
					updateFlexibleSlotStatus(Status_Reason.BLOCKED);
				else
					updateSlotStatus(Status_Reason.BLOCKED);
			}
			else if (form.getLocalContext().getCancelBlockCM() == CANCEL_SLOT)
			{
				if (form.lyrSessEdit().tabActivitiesWithSlots().grdSlotsFlexible().getSelectedRow() != null )
		...
		if (formName.equals(form.getForms().Scheduling.SessionFlexibleSlotDialog) && result.equals(DialogResult.OK))
		{

			SessionParentChildSlotVoCollection sessionSlots = form.getGlobalContext().Scheduling.getFlexibleSessionSlots();
			...
			for (int i = 0; i < sessionSlots.size(); i++)
			{
				addOutpatientFlexibleRow(sessionSlots.get(i));
			}
		}
		
		//WDEV-22343
		if (formName.equals(form.getForms().Admin.ProcedureSpecialtyHotlist))
		{
			if(result.equals(DialogResult.OK))
			{
	...
	private void addSelectedProcedures()
	{
		...
		for(ProcedureLiteVo procedure : form.getGlobalContext().Clinical.getSelectedProcedures())
		{
			...
			GenForm.lyrSessEditLayer.tabTheatreProceduresContainer.grdTheatreRow tRow = form.lyrSessEdit().tabTheatreProcedures().grdTheatre().getRows().newRow(true);
			
			tRow.setValue(new TheatreProcedureLiteVo());
			tRow.getcolProcedure().newRow(procedure, procedure.getProcedureName());
			tRow.getcolProcedure().setValue(procedure);
	...
	private void updateFlexibleSlotStatus(Status_Reason statusReason)
	{
		grdSlotsFlexibleRow selectedRow = form.lyrSessEdit().tabActivitiesWithSlots().grdSlotsFlexible().getSelectedRow();
		
		if (selectedRow.getValue() != null)
		{			
			selectedRow.getValue().setStatus(statusReason);
			selectedRow.setColStatus(getImage(statusReason));
	...
	{
		if (form.getLocalContext().getListOwnerRemoveConfirm().equals(messageBoxId))
		{
			form.getLocalContext().setListOwnerRemoveConfirm(null);
			
			if (DialogResult.YES.equals(result))
			{
//				if (canRemoveListOwner(form.lyrSessEdit().tabListOwners().grdListOwner().getValue()))
//				{
					form.lyrSessEdit().tabListOwners().grdListOwner().removeSelectedRow();
//				}
//				else
//				{
//					engine.showMessage("Cannot remove the List Owner as it is beeing used in Slots that have appointments.");
//				}
			}
	...
	//WDEV-19518 - Case Note Tracking - Modification to Profiles Management form
	@Override
	protected void onBtnCaseNoteLocationClick()	throws PresentationLogicException
	{
	...
	@Override
	protected void onGrdSlotsFlexibleSelectionChanged() throws PresentationLogicException
	{
		updateControlState();
