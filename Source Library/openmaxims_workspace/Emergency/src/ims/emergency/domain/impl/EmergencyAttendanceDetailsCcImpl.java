...
import ims.core.patient.domain.objects.NationalHealthCover;
...
import ims.core.vo.domain.NationalHealthCoverVoAssembler;
...
import ims.scheduling.vo.domain.BookingAppointmentForEDAttendanceDetailsVoAssembler;
...
public class EmergencyAttendanceDetailsCcImpl extends BaseEmergencyAttendanceDetailsCcImpl
{
	...
	public EpisodeDetailsVo save(EpisodeDetailsVo record, CareSpellVo voCareSpell, EpisodeofCareVo voEpisodeOfCare, CareContextVo voCareContext, TrackingSendToAreaVo trackingVo, TIIGVo tiigrecord, RTAVo rtaRecord, AttendanceDetailsVo emergattendance, BookingAppointmentForEDAttendanceDetailsVo bookApptToSave, EmergencyAttendanceBillingVo invoiceBilling, ChartRequestedVo chartRequested, EmergencyAttendanceNonInsuranceHealthCoverVo insuranceDetails) throws StaleObjectException  //WDEV-19060 //WDEV-22956
	{
		...
		if(trackingVo != null)
		{
			...
			if(doAttendanceDEt != null && doAttendanceDEt.getId() != null && doAttendanceDEt.getCustomID() == null)
			{
				doAttendanceDEt.setCustomID(AttendanceCustomId.getCustomId(doAttendanceDEt.getId()));
				factory.save(doAttendanceDEt);
			}
			...
			{
				doAttendanceDEt.setEndOfRegistrationDateTime(new Date()); // WDEV-23361
			...
			//WDEV-20005
			if (doTracking.getCurrentCubicle() != null)
			{
				doTracking.getCurrentCubicle().setPatient(doTracking.getPatient());
				if (doTracking.getEpisode() != null)
				{	
					doTracking.getCurrentCubicle().setEpisode(doTracking.getEpisode().getEpisodeOfCare());
				}
				doTracking.getCurrentCubicle().setTrackingArea(doTracking.getCurrentArea());
				if (doTracking.getAttendance() != null)
				{	
					doTracking.getCurrentCubicle().setAttendance(doTracking.getAttendance().getCareContext());
					doTracking.getCurrentCubicle().setAllocatedBy(doTracking.getAttendance().getRegisteringMOS());
					doTracking.getCurrentCubicle().setAllocatedCubicleDateTime(doTracking.getAttendance().getRegistrationDateTime());
				}
			}	
			//WDEV-20005 --- end			
			...
			{
				invoiceBilling.validate();
				EmergencyAttendanceBilling domainEmergAttBilling = EmergencyAttendanceBillingVoAssembler.extractEmergencyAttendanceBilling(factory,invoiceBilling);
				
				domainEmergAttBilling.setEmergencyAttendence(doAttendanceDEt);
				domainEmergAttBilling.setAttendance(doAttendanceDEt.getCareContext());
				domainEmergAttBilling.setEpisode(doAttendanceDEt.getEpisode().getEpisodeOfCare());
				domainEmergAttBilling.setPatient(doAttendanceDEt.getPatient());
			...
			{
				chartRequested.validate();
				ChartRequested domainChartRequested = ChartRequestedVoAssembler.extractChartRequested(factory,chartRequested);
				
				domainChartRequested.setEmergencyAttendence(doAttendanceDEt);
				domainChartRequested.setAttendance(doAttendanceDEt.getCareContext());
				domainChartRequested.setEpisode(doAttendanceDEt.getEpisode().getEpisodeOfCare());
				domainChartRequested.setPatient(doAttendanceDEt.getPatient());
			...
			{
				NationalHealthCover doInsurance = insuranceDetails.getInsurance()!=null ? NationalHealthCoverVoAssembler.extractNationalHealthCover(factory,insuranceDetails.getInsurance()) : null;
				PatientMedicalInsurance patientMedicalInsurance = insuranceDetails.getPatientMedicalInsurance()!=null ? PatientMedicalInsuranceVoAssembler.extractPatientMedicalInsurance(factory, insuranceDetails.getPatientMedicalInsurance()) : null;
				
				patientMedicalInsurance.setPatient(doAttendanceDEt.getPatient());
				
				doAttendanceDEt.setInsurance(doInsurance);
				doAttendanceDEt.setPatientMedicalInsurance(patientMedicalInsurance);
		...
		//WDEV-19060
		if (bookApptToSave!=null)
		{
			bookApptToSave.validate();
			Booking_Appointment doBookAppt = BookingAppointmentForEDAttendanceDetailsVoAssembler.extractBooking_Appointment(factory, bookApptToSave);
	...
	//WDEV-19060
	public BookingAppointmentForEDAttendanceDetailsVo getBookApptForLastAttendance(EmergencyAttendanceRefVo emergAttendanceRef)
	{
		if(emergAttendanceRef == null || emergAttendanceRef.getID_EmergencyAttendance() == null)
			throw new CodingRuntimeException("EmergencyAttendanceRefVo not provided");
		
		DomainFactory factory = getDomainFactory();
		
		String query = "select bookAppt from EmergencyAttendance as ea left join ea.eDClinicDetails as edc left join edc.eDClinicAppt as bookAppt where ( ea.id= :emergAttID AND bookAppt.appointmentDate = :apptDate AND bookAppt.apptStatus.id= :statusID) ";
		List<?> listBookAppt= factory.find(query, new String[] {"emergAttID", "apptDate" ,"statusID"}, new Object[] {emergAttendanceRef.getID_EmergencyAttendance(), new Date(), Status_Reason.BOOKED.getID()});
		
		if(listBookAppt == null || listBookAppt.size() == 0 )
		{
			 return null;
		}
		
		return BookingAppointmentForEDAttendanceDetailsVoAssembler.create((Booking_Appointment) listBookAppt.get(0));
	}

	//WDEV-19293
	public AttendanceDetailsVo getLastDischargedAttendance(PatientRefVo patientRef)
	{
		if(patientRef == null)
			throw new CodingRuntimeException("Patient not provided");
		...
		String hsql = 	"select attDet from EmergencyAttendance as attDet where (attDet.patient.id =:idPatient and attDet.dischargeDateTime is not null) order by attDet.dischargeDateTime desc ";
		
		List list = factory.find(hsql, new String[] {"idPatient"}, new Object[] {patientRef.getID_Patient()});
		if(list == null || list.size() == 0)
			return null;
		
		return  AttendanceDetailsVoAssembler.create((EmergencyAttendance)list.get(0));
