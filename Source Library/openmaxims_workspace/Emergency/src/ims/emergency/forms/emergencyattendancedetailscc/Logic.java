...
import ims.emergency.vo.EmergencyEpisodeTIIGVo;
...
import ims.scheduling.vo.BookingAppointmentForEDAttendanceDetailsVo;
...
public class Logic extends BaseLogic
{
	...
	private static final String PATIENT_STATUS				= "EAD04";//WDEV-19341
	...
	//WDEV-19008
	//private static final String REGISTRATION_END_TIME		= "EAD29"; //WDEV-19984
	private static final String REQUESTED_BED_TYPE 			= "EAD29"; //WDEV-19984
	private static final String USE_PRIVATE_HEALTH_INSURANCE= "EAD30"; //WDEV-19984
	private static final String ACCOMODATION_TYPE_REQUESTED = "EAD31"; //WDEV-19229 //WDEV-19984
	...
	//WDEV-19060
	private BookingAppointmentForEDAttendanceDetailsVo markApptAsSeen(BookingAppointmentForEDAttendanceDetailsVo voAppt)
	{
	...
	private TrackingSendToAreaVo populateTracking()
	{
		...
		//WDEV-20005
		if (form.getGlobalContext().Emergency.getTrackingAreaForSendToAreaVo() != null)
		{
			if (form.getGlobalContext().Emergency.getTrackingAreaForSendToAreaVo().getAllocatedCubicle() != null)
			{
				CubicleAllocationVo record = new CubicleAllocationVo();

				record.setCubicleRoomBed(form.getGlobalContext().Emergency.getTrackingAreaForSendToAreaVo().getAllocatedCubicle());
				...
				record.setAttendance(new CareContextLiteVo());// dummy population will be repopulated on Impl save method
				record.setEpisode(new EpisodeofCareVo());// dummy population
				record.setTrackingArea(new TrackingAreaLiteVo()); // dummy population

				trackingVo.setCurrentCubicle(record);
			}
			else
			{
				trackingVo.setCurrentCubicle(null);
			}
		}
		////WDEV-20005 -- end
	...
	private EpisodeDetailsVo populateDataFromScreen()
	{
		...
		voEpisodeDetails.setSchool(form.qmbSchool().getValue()); // wdev-14420
		...
		voEpisodeDetails.setTriageCategoryType(form.cmbPatientStatus().getValue()); //WDEV-19341
		...
		// Populate Ambulance Details form local context - they will be overwritten from form if that is the case
		voAttendanceDetails.setAmbulanceJobNo(form.getLocalContext().getAmbulanceDetails() != null ? form.getLocalContext().getAmbulanceDetails().getAmbulanceJobNo() : null);
		voAttendanceDetails.setAmbulanceArrivalDateTime(form.getLocalContext().getAmbulanceDetails() != null ? form.getLocalContext().getAmbulanceDetails().getAmbulanceArrivalDateTime() : null);
		voAttendanceDetails.setAmbulanceHandoverDateTime(form.getLocalContext().getAmbulanceDetails() != null ? form.getLocalContext().getAmbulanceDetails().getAmbulanceHandoverDateTime() : null);
		voAttendanceDetails.setAmbulanceDelayReason(form.getLocalContext().getAmbulanceDetails() != null ? form.getLocalContext().getAmbulanceDetails().getAmbulanceDelayReason() : null);
		voAttendanceDetails.setAmbulanceDelayReasonComment(form.getLocalContext().getAmbulanceDetails() != null ? form.getLocalContext().getAmbulanceDetails().getAmbulanceDelayReasonComment(): null); //WDEV-19284
		
		// Continue populating from screen
		...
		//WDEV-19008
		voAttendanceDetails.setBedTypeRequested(form.cmbRequestedBedType().getValue());
		voAttendanceDetails.setEndOfRegistrationDateTime(form.dtimRegistrationEndTime().getValue());
		
		if (voAttendanceDetails.getEndOfRegistrationDateTime() == null && voAttendanceDetails.getID_EmergencyAttendance() == null)
		{
			voAttendanceDetails.setEndOfRegistrationDateTime(new DateTime());
		}
		
		//
		voAttendanceDetails.setAccomodationRequestedType(form.cmbAccomodationTypeRequested().getValue());//WDEV-19229
		
		voAttendanceDetails.setUsePrivateHealthInsurance(form.chkUsePrivateHealthInsurance().getValue());
		
		//WDEV-19293
		AttendanceDetailsVo lastDischargedAtt = domain.getLastDischargedAttendance(form.getGlobalContext().Core.getPatientShort());
		Integer age = form.getGlobalContext().Core.getPatientShort().getAge();
		int hours = -1;
	
		if (lastDischargedAtt!=null && lastDischargedAtt.getDischargeDateTime()!=null)
		{
			if (voAttendanceDetails.getArrivalDateTime()!=null)
			{
				hours=hoursDiff(voAttendanceDetails.getArrivalDateTime(),lastDischargedAtt.getDischargeDateTime() );
			}
			else
			{	
				hours=hoursDiff(new DateTime(),lastDischargedAtt.getDischargeDateTime() );
			}
		}
		
		if ((age!=null && age>=17 && PresentingComplaint.NON_TRAUMATIC_CHEST_PAIN.equals(form.cmbPresComplaint().getValue())) 
			||(age!=null && age<=1 && PresentingComplaint.FEBRILE_CHILD.equals(form.cmbPresComplaint().getValue()))
			|| (GroupCategoryEnumeration.rdoUnscheduledReturn.equals(form.GroupCategory().getValue()) && hours >=0 && hours <= 72))
		{
			if (!RequiresSignOff.SIGNED_OFF.equals(voAttendanceDetails.getRequiresSignOff()))
			{
				voAttendanceDetails.setRequiresSignOff(RequiresSignOff.REQUIRES_SIGN_OFF);
			}
		}
		else
		{
			voAttendanceDetails.setRequiresSignOff(RequiresSignOff.NOT_APPLICABLE);
		}
	...
	protected void onFormDialogClosed(FormName formName, DialogResult result) throws PresentationLogicException
	{
		...
		if (formName.equals(form.getForms().Emergency.SendToAreaDialog) && result.equals(DialogResult.OK))
		{
			if (save()) 
			{
				...
				if(Boolean.TRUE.equals(form.getLocalContext().getDisplayPrintReport()))//WDEV-22943
				{
					engine.open(form.getForms().Core.PrintReport);//WDEV-17428
				}
				else
				{
					form.getLocalContext().setSelectedEvent(EmergencyAttendanceEvent.CLOSE_DIALOG);
					form.fireCustomControlValueChanged();
				}
		...
		{
			EmergencyEpisodeTIIGVo tempVo = form.getGlobalContext().Emergency.getTIIG();
			if (tempVo != null)
			{
		...
		if (formName.equals(form.getForms().Emergency.AmbulanceDetails) && DialogResult.OK.equals(result))
		{
			// Keep value in local context
			form.getLocalContext().setAmbulanceDetails(form.getGlobalContext().Emergency.getAmbulanceDetails());
			
			// Update common fields in form as well
			form.txtAmbulanceJobNo().setValue(form.getGlobalContext().Emergency.getAmbulanceDetails() != null ? form.getGlobalContext().Emergency.getAmbulanceDetails().getAmbulanceJobNo() : null);
			form.dtimAmbulanceArrivalTime().setValue(form.getGlobalContext().Emergency.getAmbulanceDetails() != null ? form.getGlobalContext().Emergency.getAmbulanceDetails().getAmbulanceArrivalDateTime() : null);
		}
		
		//WDEV-22801
		if (formName.equals(form.getForms().Core.PrintReport))
		{
			form.getLocalContext().setSelectedEvent(EmergencyAttendanceEvent.CLOSE_DIALOG);
			form.fireCustomControlValueChanged();
		}
	...
	private void clearInstanceControls()
	{
		...
		form.cmbAttendanceType().removeRow(EDVisitType.UNSCHEDULED_7_DAYS);
		form.cmbAttendanceType().removeRow(EDVisitType.UNSCHEDULED_28_DAYS);
		...
		form.dtimRegistrationEndTime().setValue(null);
		form.chkUsePrivateHealthInsurance().setValue(null);
		
		form.cmbAccomodationTypeRequested().setValue(null); //WDEV-19229
	}
	...
	private void clearEpisodeControls()
	{
		...
		form.cmbPatientStatus().setValue(null);//WDEV-19341
		...
		form.qmbSchool().clear(); //WDEV-21095
	...
	private void getAndDisplayRecord(ValueObject rowValue)
	{
		...
		if (rowValue instanceof AttendanceDetailsVo)
		{
			...
			if (GroupCategoryEnumeration.rdoUnscheduledReturn.equals(form.GroupCategory().getValue()))
			{
				form.cmbAttendanceType().newRow(EDVisitType.UNSCHEDULED_7_DAYS, EDVisitType.UNSCHEDULED_7_DAYS.getText());
				form.cmbAttendanceType().newRow(EDVisitType.UNSCHEDULED_28_DAYS, EDVisitType.UNSCHEDULED_28_DAYS.getText());
			}
			...
			if (domain.isModeOfArrivalAmbulance(attendance.getModeOfArrival()))
			{
				AmbulanceDetailsVo ambulanceDetails = new AmbulanceDetailsVo();
				ambulanceDetails.setAmbulanceArrivalDateTime(attendance.getAmbulanceArrivalDateTime());
				ambulanceDetails.setAmbulanceJobNo(attendance.getAmbulanceJobNo());
				ambulanceDetails.setAmbulanceHandoverDateTime(attendance.getAmbulanceHandoverDateTime());
				ambulanceDetails.setAmbulanceDelayReason(attendance.getAmbulanceDelayReason());
				ambulanceDetails.setAmbulanceDelayReasonComment(attendance.getAmbulanceDelayReasonComment());  //WDEV-19284
			...
			if(episode.getSchool() != null)
			{
				form.qmbSchool().newRow(episode.getSchool(), episode.getSchool().getText());
			}
			form.qmbSchool().setValue(episode.getSchool());
			...
			form.cmbPatientStatus().setValue(episode.getTriageCategoryType());//WDEV-19341
			...
			//WDEV-19008
			form.cmbRequestedBedType().setValue(attendance.getBedTypeRequested());
			form.dtimRegistrationEndTime().setValue(attendance.getEndOfRegistrationDateTime());
			form.chkUsePrivateHealthInsurance().setValue(attendance.getUsePrivateHealthInsurance());
			
			form.cmbAccomodationTypeRequested().setValue(attendance.getAccomodationRequestedType());//WDEV-19229
		}
	...
	private void updateControlState()
	{
		...
		Integer age = calculateAge(); //WDEV-23085
		int paediatricAge = ConfigFlag.GEN.PAEDIATRIC_AGE.getValue();
		...
		form.btnNew().setEnabled(FormMode.VIEW.equals(form.getMode()) && getLoc() != null && isCurrentLoggedInUserMemberOfStaff); //WDEV-18893 
		...
		form.dtimRegistrationEndTime().setRequired(true); //WDEV-19984
		form.dtimRegistrationEndTime().setEnabled(FormMode.EDIT.equals(form.getMode())); //WDEV-19984
		...
		for (int i=0;attendanceControlsConfig!=null && i<attendanceControlsConfig.size();i++)
		{
			...
			else if (PATIENT_STATUS.equals(controlConfig.getControlIMSID())) //WDEV-19341
			{
				form.lblTriageCategory().setVisible(controlConfig.getIsVisible());
				//WDEV-19341
				form.cmbPatientStatus().setVisible(controlConfig.getIsVisible());
				form.cmbPatientStatus().setRequired(controlConfig.getIsRequired());
				form.cmbPatientStatus().setEnabled(FormMode.EDIT.equals(form.getMode()) && controlConfig.getIsEditable());
				//------end WDEV-19341
			...
			else if (MODE_OF_ARRIVAL.equals(controlConfig.getControlIMSID()))
			{
				...
				form.btnAmbulanceDetails().setVisible(controlConfig.getIsVisible() && domain.isModeOfArrivalAmbulance(form.cmbModeOfArrival().getValue()));
			...
			else if (AMBULANCE_JOB_NO.equals(controlConfig.getControlIMSID()))
			{
				...
				form.lblAmbulanceJobNo().setTextColor(ModeOfArrival.AMBULANCE.equals(form.cmbModeOfArrival().getValue()) || domain.isModeOfArrivalAmbulance(form.cmbModeOfArrival().getValue()) ? Color.Red :Color.Black);
				...
				form.txtAmbulanceJobNo().setEnabled(FormMode.EDIT.equals(form.getMode()) && controlConfig.getIsEditable() && (ModeOfArrival.AMBULANCE.equals(form.cmbModeOfArrival().getValue()) || domain.isModeOfArrivalAmbulance(form.cmbModeOfArrival().getValue())));
			...
			{
				form.lblSchool().setVisible(controlConfig.getIsVisible() && age != null && age <= paediatricAge);
				
				form.qmbSchool().setVisible(controlConfig.getIsVisible() && age != null && age <= paediatricAge);
				form.qmbSchool().setRequired(controlConfig.getIsRequired());
				form.qmbSchool().setEnabled(FormMode.EDIT.equals(form.getMode()) && controlConfig.getIsEditable());
				
			}
			else if (OCCUPATION.equals(controlConfig.getControlIMSID()))
			{
				form.lblOccupation().setVisible(controlConfig.getIsVisible() && (age == null || age > paediatricAge));
				
				form.cmbOcupation().setVisible(controlConfig.getIsVisible() && (age == null || age > paediatricAge));
			...
			{
				form.lblOccupationStatus().setVisible(controlConfig.getIsVisible() && (age == null || age > paediatricAge));
				
				form.cmbOccupationStatus().setVisible(controlConfig.getIsVisible() && (age == null || age > paediatricAge));
			...
			{
				form.lblPlaceOfEmployment().setVisible(controlConfig.getIsVisible() && (age == null || age > paediatricAge));
				
				form.txtPlaceOfEmployment().setVisible(controlConfig.getIsVisible() && (age == null || age > paediatricAge));
			...
			else if (AMBULANCE_ARRIVAL_TIME.equals(controlConfig.getControlIMSID()))
			{
				...
				form.dtimAmbulanceArrivalTime().setEnabled(FormMode.EDIT.equals(form.getMode()) && controlConfig.getIsEditable()
																&& (ModeOfArrival.AMBULANCE.equals(form.cmbModeOfArrival().getValue()) || domain.isModeOfArrivalAmbulance(form.cmbModeOfArrival().getValue())));
			}
			...
			//WDEV-19008 //WDEV-19984  field will always be mandatory
			/*else if (REGISTRATION_END_TIME.equals(controlConfig.getControlIMSID()))
			{
				form.lblRegistrationEndTime().setVisible(controlConfig.getIsVisible());
				
				form.dtimRegistrationEndTime().setVisible(controlConfig.getIsVisible());
				form.dtimRegistrationEndTime().setRequired(controlConfig.getIsRequired());
				form.dtimRegistrationEndTime().setEnabled(FormMode.EDIT.equals(form.getMode()) && controlConfig.getIsEditable());			
			}*/
			else if (REQUESTED_BED_TYPE.equals(controlConfig.getControlIMSID()))
			{
				form.lblRequestedBedType().setVisible(controlConfig.getIsVisible());
				
				form.cmbRequestedBedType().setVisible(controlConfig.getIsVisible());
				form.cmbRequestedBedType().setRequired(controlConfig.getIsRequired());
				form.cmbRequestedBedType().setEnabled(FormMode.EDIT.equals(form.getMode()) && controlConfig.getIsEditable());			
			}
			else if (USE_PRIVATE_HEALTH_INSURANCE.equals(controlConfig.getControlIMSID()))
			{
				form.lblUsePrivateHealtInsurance1().setVisible(controlConfig.getIsVisible());
				form.lblUsePrivateHealtInsurance2().setVisible(controlConfig.getIsVisible());
				
				form.chkUsePrivateHealthInsurance().setVisible(controlConfig.getIsVisible());
				form.chkUsePrivateHealthInsurance().setEnabled(FormMode.EDIT.equals(form.getMode()) && controlConfig.getIsEditable());			
			}
			//WDEV-19229
			else if (ACCOMODATION_TYPE_REQUESTED.equals(controlConfig.getControlIMSID())) 
			{
				form.lblAccomodationTypeRequested1().setVisible(controlConfig.getIsVisible());
				//form.lblAccomodationTypeRequested2().setVisible(controlConfig.getIsVisible());
				
				form.cmbAccomodationTypeRequested().setVisible(controlConfig.getIsVisible());
				form.cmbAccomodationTypeRequested().setRequired(controlConfig.getIsRequired());
				form.cmbAccomodationTypeRequested().setEnabled(FormMode.EDIT.equals(form.getMode()) && controlConfig.getIsEditable());			
			}
		...
		form.btnSave().setVisible(FormMode.EDIT.equals(form.getMode()) && ((selectedRowValue != null && (selectedRowValue instanceof AttendanceDetailsVo)) || !SVUH_FUNCTIONALITY || Boolean.TRUE.equals(form.getLocalContext().getIsFinalTab())));
	...
	protected void onBtnAmbulanceDetailsClick() throws PresentationLogicException
	{
		...
		ambulanceDetails.setAmbulanceArrivalDateTime(form.dtimAmbulanceArrivalTime().getValue());
		ambulanceDetails.setAmbulanceJobNo(form.txtAmbulanceJobNo().getValue());
		
		ambulanceDetails.setArrivalDateTime(form.dtimArrival().getValue());//WDEV-20549
		
		if (form.getLocalContext().getAmbulanceDetails() != null)
		{
			ambulanceDetails.setAmbulanceHandoverDateTime(form.getLocalContext().getAmbulanceDetails().getAmbulanceHandoverDateTime());
			ambulanceDetails.setAmbulanceDelayReason(form.getLocalContext().getAmbulanceDetails().getAmbulanceDelayReason());
			ambulanceDetails.setAmbulanceDelayReasonComment(form.getLocalContext().getAmbulanceDetails().getAmbulanceDelayReasonComment()); //WDEV-19284
		}
		
		form.getGlobalContext().Emergency.setAmbulanceDetails(ambulanceDetails);
		
		engine.open(form.getForms().Emergency.AmbulanceDetails, new Object[] {form.getMode()});
	}
	
	
	// wdev-14420 //WDEV-23085 
	...
	public void newInstance()
	{
		...
		form.dtimRegistrationEndTime().setValue(dt); //WDEV-19984
		...
		//WDEV-19060
		if (form.getGlobalContext().Emergency.getBookedApptForLatestEmergencyAttendanceIsNotNull())
		{
			form.GroupCategory().setValue(GroupCategoryEnumeration.rdoScheduledReturn);
			form.cmbAttendanceType().removeRow(EDVisitType.UNSCHEDULED_7_DAYS);
			form.cmbAttendanceType().removeRow(EDVisitType.UNSCHEDULED_28_DAYS);

			form.cmbAttendanceType().setValue(EDVisitType.SCHEDULED_ATTENDENCE); 
			engine.open(form.getForms().Emergency.EmergencyAttendancesDialog); 
		}
	...
	private void enableDisableScheduledUnscheduled()
	{
		...
		if (attendVo != null)
		{
			form.getGlobalContext().Emergency.setBookedApptForLatestEmergencyAttendance(domain.getBookApptForLastAttendance(attendVo)); //WDEV-19060
			...
			if (attendVo.getArrivalDateTimeIsNotNull())
			{
				...
				if ((numberOfDaysScheduled < ConfigFlag.GEN.EMERGENCY_ATTENDANCE_SCHEDULED_RETURN_RULE_DAYS.getValue()) || form.getGlobalContext().Emergency.getBookedApptForLatestEmergencyAttendanceIsNotNull() )		//wdev-17577  //WDEV-19060
				{
	...
	{
		saveAttendance(null, null, null); //WDEV-22956
	}
	...
	private String[] getUIErrors()
	{
		...
		if (form.cmbPatientStatus().isRequired() && form.cmbPatientStatus().isEnabled() && form.cmbPatientStatus().getVisible() && form.cmbPatientStatus().getValue()==null) //WDEV-19341
		{
		...
		if (form.qmbSchool().isRequired() && form.qmbSchool().isEnabled() && form.qmbSchool().getVisible() && form.qmbSchool().getValue()==null)
		{
		...
		//WDEV-19008
		if (form.cmbRequestedBedType().isRequired() && form.cmbRequestedBedType().isEnabled() && form.cmbRequestedBedType().getVisible()&& form.cmbRequestedBedType().getValue()==null)
		{
			errors.add("Requested Bed Type is mandatory!");
		}
		if (form.dtimRegistrationEndTime().getValue() == null) //WDEV-19984
		{
			errors.add("Registration End Time is mandatory!");
		}
		else if (form.dtimRegistrationEndTime().getValue() != null && form.dtimRegistration().getValue() != null && form.dtimRegistrationEndTime().getValue().isLessThan(form.dtimRegistration().getValue()))
		{
			errors.add("Registration End Time cannot be less than Registration Time!");
		}
		
		//WDEV-19229
		if (form.cmbAccomodationTypeRequested().isRequired() && form.cmbAccomodationTypeRequested().isEnabled() && form.cmbAccomodationTypeRequested().getVisible()&& form.cmbAccomodationTypeRequested().getValue()==null)
		{
			errors.add("Accomodation Type Requested is mandatory!");
		}
	...
	{
		form.getLocalContext().setDisplayPrintReport(displayPrintReport);//WDEV-22943	
	}
