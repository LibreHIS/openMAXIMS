...
public class Logic extends BaseLogic
{
	...
	private static final Integer	COLUMN_SERVICE_LOCATION	= 0;
	
	private static final Integer	COLUMN_IS_SUBJECT_TO_RTT	= 1; //WDEV-20945
	private static final Integer	COLUMN_TREATMENT_KPI		= 2;
	private static final Integer	COLUMN_DAYS_TO_1APPT		= 3;
	...
	{
		form.getLocalContext().setRemovedCCG(null); //WDEV-19042 
		open();
	}
	...
	protected void clearScreen() 
	{
		...
		// WDEV-18797 - start
		form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getRows().clear(); //WDEV-20181
	...
	public void newInstance() throws ims.framework.exceptions.PresentationLogicException
	{
		...
		form.grdDetails().setValue(null); // WDEV-18797 
	}
	public boolean save() throws ims.framework.exceptions.PresentationLogicException
	{
		...
		if( record.getCCGsForContractIsNotNull() && record.getCCGsForContract().size() > 0 )
		{
			for( int z = 0; z < record.getCCGsForContract().size();z++)
			{
				...
				if( sscgvo != null && sscgvo.getCCGCodeIsNotNull() && !sscgvo.getCCGCode().equals(OVERSEAS_VISITOR_CCG) && !sscgvo.getCCGCode().equals(RESIDENCE_NOT_KNOWN_CCG) &&  (!sscgvo.getID_CCGToContractConfigIsNotNull() || Boolean.TRUE.equals(sscgvo.getIsEdited()))) //WDEV-20585
				{
			...
			for( int g = 0; g < form.ctnDetails().lyrDetails().tabPageCCG().grdCCG().getRows().size(); g++)
			{
				...
				for( int q = 0; q < record.getCCGsForContract().size();q++)
				{
					...
					if( sscgvo != null && sscgvo.getCCGCode() != null && sscgvo.getCCGCode() != "" && Boolean.TRUE.equals(sscgvo.getIsActive()) && sscgvo.getCCGCode().equals(form.ctnDetails().lyrDetails().tabPageCCG().grdCCG().getRows().get(g).getColumnCCG()))
					{
	...
	public void updateControlsState()
	{
		...
		form.ctnDetails().lyrDetails().tabPageCCG().setHeaderVisible(Boolean.TRUE.equals(ConfigFlag.GEN.CONTRACT_CCG_ENABLED.getValue()));
		form.ctnDetails().lyrDetails().tabPage2().dyngrdService().setReadOnly(form.getMode().equals(FormMode.VIEW));//WDEV-20181
	}
	...
	public void initialize() throws FormOpenException 
	{
		...
		initializeServiceDynGrid();//WDEV-20181
		
	}
	
	//WDEV-20181
	private void initializeServiceDynGrid()
	{
		form.ctnDetails().lyrDetails().tabPage2().dyngrdService().clear();

		//form.dyngrdPatientCodingList().setHeaderHeight(50);
		
		DynamicGridColumn columnName = form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getColumns().newColumn("Service/Location", COLUMN_SERVICE_LOCATION);
		columnName.setCanGrow(true);
		columnName.setWidth(150); //WDEV-20945
		
		//WDEV-20945
		columnName = form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getColumns().newColumn("RTT", COLUMN_IS_SUBJECT_TO_RTT);
		columnName.setCanGrow(true);
		columnName.setWidth(24);	
		//WDEV-20945 - ends here
		
		columnName = form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getColumns().newColumn("Week(s) to Treatment KPI", COLUMN_TREATMENT_KPI); //WDEV-20945
		columnName.setCanGrow(true);
		columnName.setWidth(100);
		
		columnName = form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getColumns().newColumn("No. of Days to 1st Activity Appointment", COLUMN_DAYS_TO_1APPT); //WDEV-20945
		columnName.setCanGrow(true);
	...
	protected ContractConfigVo populateDataFromScreen(ContractConfigVo value) 
	{
		...
		if(tempVoColl != null)
		{
			for(int i = 0 ;i<tempVoColl.size();i++)
			{
				...
				DynamicGridRow row=getRowFromDynGrid(tempVo);
				Integer daysToRttBreach=(Integer)row.getCells().get(getColumn(COLUMN_TREATMENT_KPI)).getValue();
				tempVo.setDaysToRTTBreachDate(daysToRttBreach!=null &&  daysToRttBreach>0 ? daysToRttBreach*7 : null );
				tempVo.setDaysTo1stAppt((Integer)row.getCells().get(getColumn(COLUMN_DAYS_TO_1APPT)).getValue());
	...
	private DynamicGridRow getRowFromDynGrid(ContractServiceLocationsConfigVo tempVo)
	{
		if (tempVo==null)
			return null;
		...
		{
			if (tempVo.getService()!=null && tempVo.getService().equals(form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getRows().get(i).getValue()))
			{
				return form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getRows().get(i);
			}
		}
...
//			errs.add("Weeks To RTT Breach Date must be one or greater");
			errs.add("Week(s) to Treatment KPI must be one or greater");
		}
...
//			errs.add("Number of days to 1st Appointment must be 0 or greater");
			errs.add("No. of Days to 1st Activity Appointment must be 0 or greater");
		}
		//wdev-12676
		...
		//WDEV-18508 WDEV-19171
		if(Boolean.TRUE.equals(ConfigFlag.GEN.CONTRACT_CCG_ENABLED.getValue()) && PreActiveActiveInactiveStatus.ACTIVE.equals(form.ctnDetails().lyrDetails().tabPage1().cmbStatus().getValue()) && form.ctnDetails().lyrDetails().tabPageCCG().grdCCG().getRows().size() < 1)
		{
			errs.add("CCG Code is mandatory");
		}
	...
	protected void onContextMenuItemClick(int menuItemID, Control sender) throws PresentationLogicException 
	{
		switch (menuItemID)
		{
			...
			//WDEV-20945
			case ims.RefMan.forms.contractconfiguration.GenForm.ContextMenus.RefManNamespace.ServiceLocationMenu.AddEditServiceKpi:
				addEditServiceKpi();
				break;
			//WDEV-20945 ends here
	...
	private void addEditServiceKpi() 
	{
		ServiceLiteVo serviceVo = (ServiceLiteVo) form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getValue();
		ContractServiceLocationsConfigVoCollection tempVoColl = form.getLocalContext().getServiceAndLocation();
		for(ContractServiceLocationsConfigVo tempVo:tempVoColl)
		{
			if (serviceVo.getID_Service().equals(tempVo.getService().getID_Service()))
			{
				form.getGlobalContext().RefMan.setSelectedServiceForKpi(tempVo);
				break;
			}
		}
		engine.open(form.getForms().RefMan.ServiceKPIs);
	}
	
	//wdev-13616
	private void removeLocation() //WDEV-20181
	{
		if(form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getSelectedRow()!= null && form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getSelectedRow().getValue() instanceof ConfigLocationLiteVo)
		{
			ServiceLiteVo serVo = null;
			DynamicGridRow parent = form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getSelectedRow();
			while(parent.getParent() != null)
				parent = parent.getParent();
			if(parent.getValue() instanceof ServiceLiteVo)
				serVo = (ServiceLiteVo) parent.getValue();
				
				
			ConfigLocationLiteVo locationVo = (ConfigLocationLiteVo) form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getSelectedRow().getValue();
	...
	protected void onFormDialogClosed(FormName formName, DialogResult result) throws PresentationLogicException 
	{
		...
		{
			populateParentsDynGrid(form.getGlobalContext().RefMan.getServiceContrat());//WDEV-20181
			updateMenuContext();
		}
		else if(formName.equals(form.getForms().Admin.LocationMultipleSelectDialog) && result.equals(DialogResult.OK))
		{
			populateChildsDynGrid(form.getGlobalContext().RefMan.getLocationContract());//WDEV-20181
			displayDynGrid();
			updateMenuContext();
		}
		else if (formName.equals(form.getForms().RefMan.ServiceKPIs) && result.equals(DialogResult.OK))
		{
			if (form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getValue() instanceof ServiceLiteVo && ((ServiceLiteVo) form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getValue()).getID_Service().equals(form.getGlobalContext().RefMan.getSelectedServiceForKpi().getService().getID_Service()))
			{
				form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getSelectedRow().getCells().get(getColumn(COLUMN_TREATMENT_KPI)).setValue(form.getGlobalContext().RefMan.getSelectedServiceForKpi().getDaysToRTTBreachDate() != null ? form.getGlobalContext().RefMan.getSelectedServiceForKpi().getDaysToRTTBreachDate()/DAYS_FOR_WEEK : null);
				form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getSelectedRow().getCells().get(getColumn(COLUMN_DAYS_TO_1APPT)).setValue(form.getGlobalContext().RefMan.getSelectedServiceForKpi().getDaysTo1stAppt() != null ? form.getGlobalContext().RefMan.getSelectedServiceForKpi().getDaysTo1stAppt() : null);
			}
			updateMenuContext();
		}
	}
	...
	//WDEV-20181
	private DynamicGridRow addParentRowInServiceDynGrid(ContractServiceLocationsConfigVo tempContractVo)
	{
		DynamicGridRow newRow = form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getRows().newRow();
		
		newRow.setValue(tempContractVo.getService());
		
		DynamicGridCell cellService = newRow.getCells().newCell(getColumn(COLUMN_SERVICE_LOCATION), DynamicCellType.STRING);
		cellService.setValue(tempContractVo.getService().getServiceName());
		cellService.setReadOnly(true);
		
		
		
		DynamicGridCell rttImageCell = newRow.getCells().newCell(getColumn(COLUMN_IS_SUBJECT_TO_RTT), DynamicCellType.IMAGE);
		rttImageCell.setValue(Boolean.TRUE.equals(tempContractVo.getService().getIsSubjectToRTTClock()) ? form.getImages().Core.RTTClock : null);
		
		DynamicGridCell cellRttBreach = newRow.getCells().newCell(getColumn(COLUMN_TREATMENT_KPI), DynamicCellType.INT);
		cellRttBreach.setValue(tempContractVo.getDaysToRTTBreachDate()!=null ? tempContractVo.getDaysToRTTBreachDate()/7 : null);
		cellRttBreach.setIntMaxLength(4);
		cellRttBreach.setReadOnly(true); //WDEV-20945
		
		DynamicGridCell cellFirstAppt = newRow.getCells().newCell(getColumn(COLUMN_DAYS_TO_1APPT), DynamicCellType.INT);
		cellFirstAppt.setValue(tempContractVo.getDaysTo1stAppt());
		cellFirstAppt.setIntMaxLength(4);
		cellFirstAppt.setReadOnly(true); //WDEV-20945
	...
	//wdev-13616  //WDEV-20181
	private void populateParentsDynGrid(ServiceLiteVoCollection services)
	{
		if(services == null)
			return;
		for(int i = 0; i < services.size();i++ )
		{
			if(checkIfParentAlreadyExistsinDynGrid(services.get(i)))
				continue;
			
			ContractServiceLocationsConfigVo tempContractVo = new ContractServiceLocationsConfigVo();
			ServiceLiteVo temVo = services.get(i);
			...
			addParentRowInServiceDynGrid(tempContractVo);
	...
	//wdev-13616
	private void removeService() //WDEV-20181
	{
		if(form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getValue() instanceof ServiceLiteVo)
		{
			ServiceLiteVo serviceVo = (ServiceLiteVo) form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getValue();
			ContractServiceLocationsConfigVoCollection tempVoColl = form.getLocalContext().getServiceAndLocation();
			boolean deleted = true;
	...
	private void displayDynGrid() //WDEV-20181
	{
		ServiceLiteVoCollection serviceCol = new ServiceLiteVoCollection();
		ContractServiceLocationsConfigVoCollection servicelocationVoCol = form.getLocalContext().getServiceAndLocation();
		form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getRows().clear();
		...
		for(int i = 0; i < servicelocationVoCol.size();i++)
		{
			...
			else
			{
				...
				DynamicGridRow parentRow = addParentRowInServiceDynGrid(servicelocationVoCol.get(i));
				...
				for(int j = 0; j < servicelocationVoCol.size();j++)
				{
					if(servicelocationVoCol.get(i).getServiceIsNotNull() && servicelocationVoCol.get(j).getServiceIsNotNull())
					{
						if(servicelocationVoCol.get(i).getService().getID_Service().equals(servicelocationVoCol.get(j).getService().getID_Service()))
						{
							...
							{
								addLocationRow(parentRow, servicelocationVoCol.get(j).getLocation());								
							}
						}
					}
				}
		...
		form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getRows().expandAll();
	}
	...
	//wdev-13616
	private boolean checkSerices(ServiceLiteVo service, ServiceLiteVoCollection servicecol)
	{
	...
	//WDEV-20181
	@Override
	protected void onDyngrdServiceRowSelectionChanged(DynamicGridRow row) throws PresentationLogicException
	{
		updateMenuContext();
	}
	...
	{
		return form.ctnDetails().lyrDetails().tabPage2().dyngrdService().getColumns().getByIdentifier(identifier);
	}
