...
public class ClinicListImpl extends DomainImpl implements ims.RefMan.domain.ClinicList, ims.domain.impl.Transactional
{
	...
	public ProfileShortVoCollection listProfiles(ILocation location, Date date) 
	{
		...
		condStr.append(andStr).append(" profile.profileType.id = :thrProfile ");
	...
	public CatsReferralRefVo getCatsReferralForAppointment(Booking_AppointmentRefVo appt)
	{
		...
		DomainFactory factory = getDomainFactory();
		Integer ver = new Integer(0);//WDEV-20101
		List<?> catsRefId = factory.find("select catsRef.id,catsRef.version from CatsReferral as catsRef left join catsRef.appointments as bookAppt where (bookAppt.id = '" + appt.getID_Booking_Appointment() + "')");
		if (catsRefId != null && !catsRefId.isEmpty()) //WDEV-20101
		{
			Integer val = (Integer) catsRefId.get(0);
			if (catsRefId.size() > 1)
			{	
				ver = (Integer) catsRefId.get(1);
			}
			if(val != null && val.intValue() > 0)
			{
				return new CatsReferralRefVo(val, ver); //WDEV-20101
			}
		}	
		return null;
	}
	...
	//WDEV_12568
	public void saveAppointmentForDNA(Booking_AppointmentVo appt, CatsReferralForClinicListVo catsReferral,	SessionSlotClinicListListVo sessionSlot, PatientElectiveListForDNAAppointmentsVo electiveList,	Boolean isFirstApptActivityforDNA, Boolean isUndoArrival) throws StaleObjectException, DomainInterfaceException
	{
		if(appt == null)
		...
		Booking_Appointment doAppt = Booking_AppointmentVoAssembler.extractBooking_Appointment(factory, appt);
		CatsReferral doCats = (CatsReferral) factory.getDomainObject(CatsReferral.class, catsReferral.getID_CatsReferral()); //WDEV-19772
		doCats.setConsultationActivityRequired(catsReferral.getConsultationActivityRequired());//WDEV-20748
		
		//WDEV-18524 //WDEV-19772 
		updateCatsReferral(factory, doCats, doAppt, isFirstApptActivityforDNA);
		...
		createRttStatusAndEvent(doCats,doAppt, isFirstApptActivityforDNA, isUndoArrival, eventDateTime);
	...
	private PathwayRTTStatus createPathwayRTTStatus(CatsReferral record, Boolean isBasedOnHardCodedEvent, java.util.Date eventDateTime)
	{
		...
		RTTStatusPoint rttStatusPoint =Boolean.TRUE.equals(isBasedOnHardCodedEvent) ?  getRTTStatusPointForDNAAppt(HardCodedEvents.APPOINTMENTDNA.getID()) : getRTTStatusPointFromConfig(dnaNatCode);
	...
	private void updateCatsReferral(DomainFactory factory, CatsReferral doCats, Booking_Appointment doAppt, Boolean isFirstApptActivityforDNA) throws StaleObjectException
	{
