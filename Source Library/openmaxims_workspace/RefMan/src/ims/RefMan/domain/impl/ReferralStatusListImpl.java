...
import ims.RefMan.domain.ReferralServiceSelectionDialog;
...
public class ReferralStatusListImpl extends BaseReferralStatusListImpl implements PatientReferralStatusList, IUndoEndOfTreatment
{
	...
	*/
	public DirectoryOfServiceLiteVoCollection listDirectoryOfService(String searchCriteria)
	{
	...
	public CatsReferralListManualAssemblyVoCollection listCATSReferrals(CATSReferralFilterVo voCATSFilter, Boolean bIncludeICPColumn, Boolean bIncludeELEColumn) 
	{
		...
		if (voCATSFilter.getServiceIsNotNull() || (voCATSFilter.getServices() != null) && voCATSFilter.getServices().size() > 0)	//wdev-19539
		{
			sb.append(andStr + " ( (refDetails.service.id in (" + getServicesIds(voCATSFilter) + ") ) ");	//wdev-19539
		...
		{
			hqlJoin += " join catsref.patient.identifiers as patids left join patids.type as patidsType ";
			
			sb.append(andStr + " patidsType.id = :idType");
			markers.add("idType");
			values.add(PatIdType.NHSN.getID());
			andStr = " and ";
		...
		{
			sb.append(andStr + " catsref.currentStatus.referralStatus.id != :providerCancellation ");
			markers.add("providerCancellation");
			values.add(ReferralApptStatus.REFERRAL_CANCELLED_BY_PROVIDER.getID());
			andStr = " and ";
		}
		...
		//WDEV-23943
		if (voCATSFilter.getResponsibleHCP() != null && (Boolean.TRUE.equals(voCATSFilter.getNoResponsibleHCP())))
		{
			sb.append(andStr +  " ( refDetails != null and (cons.id= :respHCP OR cons is null) ) ");
			markers.add("respHCP");
			values.add(voCATSFilter.getResponsibleHCP().getIMosHcpId());	
			andStr = " and ";
		}
		else if (Boolean.TRUE.equals(voCATSFilter.getNoResponsibleHCP()))
		{
			sb.append(andStr +  " ( refDetails != null and cons is null ) ");	
			andStr = " and ";
		}
		//WDEV-19327 
		else if (voCATSFilter.getResponsibleHCP() != null)
		{
			sb.append(andStr +  " ( refDetails != null and cons.id= :respHCP ) ");
			markers.add("respHCP");
			values.add(voCATSFilter.getResponsibleHCP().getIMosHcpId());	
			andStr = " and ";
		}		
		...
		if(voCATSFilter.getStatus() != null 
				&& !ReferralWorklistStatus.DNA_NOT_SEEN_DISCHARGED_TO_GP.equals(voCATSFilter.getStatus()) 
				&& !ReferralWorklistStatus.AWAITING_TRIAGE.equals(voCATSFilter.getStatus()) 
				&& !ReferralWorklistStatus.PROVISIONAL_REJECTION.equals(voCATSFilter.getStatus()) 
				&& !ReferralWorklistStatus.REFERRAL_ACCEPTED.equals(voCATSFilter.getStatus()) 
				&& !ReferralWorklistStatus.REFERRAL_REJECTED.equals(voCATSFilter.getStatus()) 
				&& !ReferralWorklistStatus.PROVIDER_CANCELLATION.equals(voCATSFilter.getStatus()) 
				&& !ReferralWorklistStatus.FURTHER_MANAGMENT_REQUIRED.equals(voCATSFilter.getStatus()) 
				&& !ReferralWorklistStatus.FIT_FOR_SURGERY_NEED_APPT.equals(voCATSFilter.getStatus()) 
				&& !ReferralWorklistStatus.CAB_REFERRALS_TO_BE_ACCEPTED_ON_CAB.equals(voCATSFilter.getStatus()) 
				&& !ReferralWorklistStatus.ONWARD_REFERRAL_WORKLIST.equals(voCATSFilter.getStatus()) 
				&& !ReferralWorklistStatus.FURTHER_MANAGEMENT_REQUIRED_TLT.equals(voCATSFilter.getStatus()) 
				&& !ReferralWorklistStatus.ACTIVE_MONITORING_REQ_APPOINTMENT.equals(voCATSFilter.getStatus()) 
				&& !ReferralWorklistStatus.ACTIVE_MONITORING.equals(voCATSFilter.getStatus()) 
				&& !ReferralWorklistStatus.REFERRAL_TRIAGE_WORKLIST.equals(voCATSFilter.getStatus()) //WDEV-22376
				&& !ReferralWorklistStatus.CONSULTATION_ACTIVITY_REQUIRED.equals(voCATSFilter.getStatus())) //WDEV-23047
		{
		...
		if (voCATSFilter.getStatusIsNotNull())
		{
			...
			*/
			else if (voCATSFilter.getStatus().equals(ReferralWorklistStatus.REBOOKING_OF_SUBSEQUENT_ACTIVITY))
				refList = domListRebookingOfSubsequestActivityAppointments(hqlSB, sb, markers, values, andStr, hqlJoin);
			...
			else if (voCATSFilter.getStatus().equals(ReferralWorklistStatus.DNA_NOT_SEEN_DISCHARGED_TO_GP))
				refList = domListDnaNotSeenDischargeToGPWorklist(hqlSB, sb, markers, values, andStr, hqlJoin); //WDEV-22376
			...
			else if (voCATSFilter.getStatus().equals(ReferralWorklistStatus.CAB_REDIRECT_SERVICE)) 
				refList = domListCABRedirectService(hqlSB, sb, markers, values, andStr, hqlJoin);
	...
	private List<?> domListCABRedirectService(String hql, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, String hqlJoin)
	{
		if (hqlJoin != "")
			hql += hqlJoin;
	...
	private List<?> domListRebookingOfSubsequestActivityAppointments(String hql, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, String hqlJoin)
	{
	...
	private List<?> domListDnaNotSeenDischargeToGPWorklist(String hql, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, String hqlJoin)
	{
	...
	private String getReferralUrgenciesForSearch(ReferralUrgencyCollection referralUrgencies)
	{
		...
		{			
			sb.append(delimiter).append(referralUrgencies.get(i).getID());
			delimiter = ",";
		}
	...
	private List<?> domListReferralConsultationActivityRequiredrefList(String hql, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, String hqlJoin)
	{
		...
		values.add(ReferralApptStatus.END_OF_CARE.getID());
		markers.add("ID2");
		values.add(ReferralApptStatus.DISCHARGED.getID());
		markers.add("ID3");
		values.add(ReferralApptStatus.REFERRAL_CANCELLED_BY_PROVIDER.getID());
		markers.add("ID4");
		values.add(ReferralApptStatus.REFERRAL_REJECTED.getID());
		markers.add("ID5");
		values.add(ReferralApptStatus.CAB_CANCELLATION.getID());	
	...
	private List<?> domListReferralTriageWorklist(String hql, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, String hqlJoin)
	{
		...
		sb.append(andStr + " ( catsref.currentStatus.referralStatus.id = :ID ");
		markers.add("ID");
		values.add(ReferralApptStatus.ONWARDREFERRALREQUESTED.getID());
	...
	private String getBaseJoinQuery() 
	{
		return 	"left join catsref.patient as pat1 " +
			...
			"left join catsref.iCABReferral as ICABDetails " +
	...
	private List<?> domListActiveMonitoringWorklist(String hql, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, String hqlJoin) 
	{
		...
		markers.add("ID");
		values.add(ReferralApptStatus.ACTIVE_MONITORING.getID());
	...
	//	"left join ra.furtherManagementDetails as fmd "; 
		...
		sb.append(andStr + " catsref.currentStatus.referralStatus.id = :id1");
		sb.append(" and catsref.wasFurtherManagementDetailsReviewed = null");
		sb.append(" and fmd.followupType.id = :id2");
	
		markers.add("id1");
		values.add(ReferralApptStatus.FURTHER_MANAGEMENT_REQUIRED.getID());
		markers.add("id2");
		values.add(FollowupManagementType.REFER_BACK_TO_CONSULTANT.getID());
	...
	private List<?> domListNotFitOrUnsuitableForSurgery(String hql, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, String hqlJoin, TLTType type) 
	{
		...
		sb.append(" and catsref.currentStatus.referralStatus.id not in (:id3) ");
	...
	private List<?> domListOnwardReferralWorklist(Boolean includeICPColumn, Boolean bIncludeELEColumn, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, String hqlJoin, OnwardReferralReason onwardReferralReason) 
	{
		...
		sb.append(andStr + " catsref.currentStatus.referralStatus.id = :id1");
		sb.append(" and rd.dischargeType.id = :id2");
		markers.add("id1");
		values.add(ReferralApptStatus.DISCHARGED.getID());
		markers.add("id2");
		values.add(DischargeType.SEC_CARE.getID());
	
		if (onwardReferralReason != null)
		{
			sb.append(" and catsref.onwardReferralReason.id = :id3");
			markers.add("id3");
			values.add(onwardReferralReason.getID());
		}
		else
	...
	private CatsReferralListManualAssemblyVoCollection assembleRecords(List<?> refList) 
	{
		...
		for (int i = 0 ; i < refList.size(); i ++)
		{
			...
			voItem.setReferralDate(doCats.getReferralDate()!= null ? new ims.framework.utils.Date(doCats.getReferralDate()) : null); //WDEV-20752
			//WDEV-19665 
	...
	//	String hql = "select catsref from CatsReferral as catsref left join catsref.referralDetails as refDetails "; 
		...
		{
			sb.append(andStr + " catsref.isFinalReportRequired.id = :isRequired");

			markers.add("isRequired");
			values.add(ReportStatus.REQUIRED.getID());
		}
		...
		{
			sb.append(andStr + " catsref.isSecondaryReportRequired.id = :isRequired");				
	...
	private List<?> domListRefManReportsRequiredWorklist(String hql, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, String hqlJoin, ReportNoteType type)
	{
		...
		if (type != null)
		{
			sb.append(andStr + " rr.reportsRequired.id = :rrID");
			markers.add("rrID");
	...
	private List<?> domListOrderInvAcceptedApptRequired(String hql, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, String hqlJoin)
	{
		...
		sb.append(andStr + "orderInv.ordInvCurrentStatus.ordInvStatus.id  = :acceptedStatus ");				
	...
	private List<?> domListFinalDischargeDocumentRequired(String hql, StringBuffer sb,ArrayList<String> markers, ArrayList<Serializable> values,String andStr, String hqlJoin) 
	{
		...
		sb.append(andStr + " catsref.isFinalReportRequired.id = :isFinalReportRequired");				
	...
	private List<?> domListFurtherManagementRequired(Boolean includeICPColumn, Boolean bIncludeELEColumn, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, String hqlJoin) 
	{
		...
		sb.append(andStr + " catsref.currentStatus.referralStatus.id = :id1");
		sb.append(" and catsref.wasFurtherManagementDetailsReviewed = null");
		sb.append(" and fmd.followupType.id not in (:id2, :id3)");//wdev-8482
		...
		values.add(ReferralApptStatus.FURTHER_MANAGEMENT_REQUIRED.getID());
		markers.add("id2");
		values.add(FollowupManagementType.TLT.getID());
	...
	private List<?> domListFurtherManagementTLTRequired(Boolean includeICPColumn, Boolean bIncludeELEColumn, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, String hqlJoin, TLTType type) 
	{
		...
		sb.append(" and fmd.followupType.id = :id3");
		markers.add("id3");
		values.add(FollowupManagementType.TLT.getID());
		
		if (type != null)
		{
			sb.append(" and fmd.tLTType.id = :id2");//wdev-8482
			markers.add("id2");
			values.add(type.getID());
		}
...
//		or invs.ordInvCurrentStatus.ordInvStatus in ORDERED or SENT or RECEIVED or ACCEPTED
		...
		sb.append(andStr + " invs.ordInvCurrentStatus.ordInvStatus.id in (:id1, :id2, :id3, :id4)");
	...
	//	Episode Status	Accepted	Accepted	Accepted	Accepted	Accepted	Accepted	Accepted
		...
		sb.append(andStr + " catsref.currentStatus.referralStatus.id not in (:id1, :id2, :id3, :id4) " +
				" and (catsref.additionalInvApptsStatus.id in (:id5, :id6) )");//WDEV-22375 or  catsref.hasCancelledApptsForReview = :isTRUE or catsref.hasDNAApptsForReview = :isTRUE)");
...
//		values.add(Status_Reason.CANCELLED.getID());
		...
		markers.add("id5");		
		values.add(AdditionalInvestigationAppointmentsStatus.TOBEBOOKED.getID());
		markers.add("id6");
		values.add(AdditionalInvestigationAppointmentsStatus.DNA.getID());
	
//		hql+= " left join catsref.appointments as appt "; WDEV-12052
	...
	private List<?> domListReferralAccepted(String hql, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, String hqlJoin) 
	{
		...
		sb.append(andStr + " catsref.hasReferralLetter = :b2" +
				" and ( catsref.consultationAppt.apptStatus.id = :id1" +
				" or catsref.consultationAppt.apptStatus = null )" +
				" and (catsref.additionalInvApptsStatus.id = :id2" +
				" or catsref.additionalInvApptsStatus.id = :id4 )" + //added upon Annes advice.
				" and catsref.currentStatus.referralStatus.id = :id3");
		...
		markers.add("id2");
		values.add(AdditionalInvestigationAppointmentsStatus.ALLBOOKED.getID());
		markers.add("id4");
		values.add(AdditionalInvestigationAppointmentsStatus.NONE.getID());
		markers.add("id3");
		values.add(ReferralApptStatus.REFERRAL_ACCEPTED.getID());
	...
	private List<?> domListProvisionalRejection(String hql, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, SecondOpinionCategory category, String hqlJoin) 
	{
		...
		sb.append(andStr + " catsref.currentStatus.referralStatus.id = :id1");
		markers.add("id1");
		values.add(ReferralApptStatus.PROVISIONAL_REJECTION.getID());
		if ( category != null)
		{
			andStr = " and ";
			sb.append(andStr + " catsref.currentStatus.secondOpinionCategory.id = :id2");
			markers.add("id2");
	...
	private List<?> domListOtherApptsRequired(String hql, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, String hqlJoin) 
	{
		...
		sb.append(" and catsref.isRIE is null and catsref.currentStatus.referralStatus.id not in (:id5, :id2, :id3, :id4)" );
	...
	private List<?> domListConsAppointmentRequired(String hql, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, String hqlJoin) 
	{
		...
		sb.append(andStr + " catsref.hasReferralLetter = :b1" +
				" and catsref.consultationAppt is null and catsref.currentStatus.referralStatus.id not in (:id1, :id2, :id3, :id4, :id5, :id6, :id7)");
	...
	private List<?> domListAwaitingReferralLetter(String hql, StringBuffer sb, ArrayList<String> markers, ArrayList<Serializable> values, String andStr, String hqlJoin) 
	{
		...
		sb.append(andStr + " ( catsref.hasReferralLetter = :b1 or catsref.hasReferralLetter = null ) and catsref.currentStatus.referralStatus.id not in (:id1, :id2, :id3, :id4)");
	...
	{
		if(voReferralRef == null || voReferralRef.getID_CatsReferral() == null)
			return null;
		
		CatsReferral doCatsReferral = (CatsReferral)getDomainFactory().getDomainObject(CatsReferral.class, voReferralRef.getID_CatsReferral());
		
		boolean hasMultipleReferralLetter = false;
		
		if(doCatsReferral != null && doCatsReferral.getReferralDocuments() != null)
		{
			Iterator it = doCatsReferral.getReferralDocuments().iterator();
			int noOfReferralLetters = 0;
			
			while(it.hasNext())
			{
				PatientDocument doc = (PatientDocument) it.next();
				
				if(DocumentCategory.GPLETTER.getID() == doc.getCategory().getId())
				{
					noOfReferralLetters++;
				}
			}
			
			hasMultipleReferralLetter = noOfReferralLetters > 1 ? true : false;
		}
		
		CatsReferralListVo catsReferral = CatsReferralListVoAssembler.create(doCatsReferral);
		if(catsReferral != null)
		{
			catsReferral.setHasMultipleReferralLetter(hasMultipleReferralLetter);
		}
	...
	public LocationLiteVoCollection listLocationLite(String locationName)
	{
		DirectoryOfServiceAdmin impl = (DirectoryOfServiceAdmin) getDomainImpl(DirectoryOfServiceAdminImpl.class);
		return impl.listLocationLiteByName(locationName);
	}
	...
	public CatsReferralWizardVo resetReferralRejection(CatsReferralWizardVo catsReferral, RejectReferralVo rejRefVo) throws StaleObjectException, ForeignKeyViolationException 
	{
		...
		PatientPathwayJourney journey = getPatientJourney(catsReferral);
		if(journey != null)
		{
			...
			doCats.setCurrentRTTStatus(currentRTTStatus);
	...
	public Boolean hadPreviousAdmissionForThisReferral(CatsReferralRefVo voCatsRef) 
	{
		...
		criteria.addOrder(DischargeSummaryPacuAndWard.FieldNames.DischargeDateTime, ORDERMODE.DESC);
	...
	public void undoReferralRejection(CatsReferralListVo catsRef) throws StaleObjectException,ForeignKeyViolationException
	{
		...
		PatientPathwayJourney journey = getPatientJourney(catsRef);
		if(journey != null)
		{
			PathwayClock finalClock = journey.getCurrentClock();
			PathwayRTTStatus currentRTTStatus = null;
			...
			referralBo.setCurrentRTTStatus(currentRTTStatus);
	...
	private PatientPathwayJourney getPatientJourney(CatsReferralRefVo catsReferral)
	{
		...
		return doCatsReferral.getJourney();
	}
	...
	public void undoEndOfCare(CatsReferralListVo catsRef)throws StaleObjectException, ForeignKeyViolationException 
	{
		...
		//Remove the current status from the history and set the current status to the latest
		history.remove(removeStatus);
		catsRef.setCurrentStatus(getLatestStatusForUndoEndOfCare(history));
	...
	private CatsReferralStatusVo getLatestStatusForUndoEndOfCare(CatsReferralStatusVoCollection statusHistory)
	{
		if (statusHistory == null || statusHistory.size() == 0)
			throw new DomainRuntimeException("Referral's history doesn't contain any records");
		
		CatsReferralStatusVo latest = null;
		for (int i  = 0 ; i < statusHistory.size() ; i++)
		{
			if ((latest == null || statusHistory.get(i).getStatusDateTime().isGreaterThan(latest.getStatusDateTime())) && !ReferralApptStatus.FIRST_CONSULTATION_APPOINTMENT_CANCELLED.equals(statusHistory.get(i).getReferralStatus()))
			{
				latest = statusHistory.get(i);
			}
		}
	...
	{
		ReferralServiceSelectionDialog impl = (ReferralServiceSelectionDialog) getDomainImpl(ReferralServiceSelectionDialogImpl.class);
		return impl.getReferralServices(serviceName, null);
	}


	public LocationLiteVo getLocationLite(LocSiteRefVo locSite)
	{
		if(locSite == null || locSite.getID_Location() == null)
			return null;
		
		return LocationLiteVoAssembler.create((Location) getDomainFactory().getDomainObject(Location.class, locSite.getID_Location()));
	}

	//WDEV-20774
	public CatsReferralForLinkRefVo getReferralLinkedToThis(Integer referralId) 
	{
		if (referralId == null)
			return null;
		
		return CatsReferralForLinkRefVoAssembler.create((CatsReferral)getDomainFactory().getDomainObject(CatsReferral.class,referralId));
	...
	public CatsReferralForLinkRefVo getParentReferralLinkedToThis(Integer refId)
	{
		if( refId == null)
			return null;
	...
	public Boolean hasStatusToRevert(CatsReferralRefVo ref)
	{
		...
		StringBuilder query = new StringBuilder("SELECT count(statusHist.id) FROM CatsReferral AS ref LEFT JOIN ref.statusHistory AS statusHist ");
		query.append(" where ");
		query.append(" ref.id = :catsReferralID ");
		
		long count = getDomainFactory().countWithHQL(query.toString(), new String[]{"catsReferralID"}, new Object[] {ref.getID_CatsReferral()});
