...
import ims.scheduling.vo.TheatreDetailVoCollection;
...
import ims.scheduling.vo.domain.TheatreDetailVoAssembler;
...
public class BookTheatreImpl extends BaseBookTheatreImpl
{
	...
	public ims.scheduling.vo.Sch_BookingTheatreVo saveTheatreBooking(ims.scheduling.vo.Sch_BookingTheatreVo voBooking, CatsReferralRefVo catsRef, SessionTheatreTCISlotLiteVo voSessTheatreSlot,TheatreType theatreType) throws ims.domain.exceptions.DomainInterfaceException, ims.domain.exceptions.StaleObjectException
	{
		...
		if (doCatsReferral != null)
		{
			...
			//SessionAdmin impl = (SessionAdmin) getDomainImpl(SessionAdminImpl.class);//WDEV-23545 - this method should never be called for Theatre Appointment
	 ...
	 */
	public Boolean hasBookingRights(IAppRole role, Sch_SessionRefVo session) //WDEV-21137
	{
		...
		if (doCatsRef != null)
		{
			...
			if (voReferralLetterDetail != null)
			{
				...
				if (voReferralLetterDetail.getDateOfReferralIsNotNull())
				{
					...
					if(voReferralLetterDetail.getDaysToRTTBreachDate() != null)
					{
	...
	public TheatreDetailVoCollection getTheatreDetailsForSession(Sch_SessionRefVo session)
	{
		if(session == null || session.getID_Sch_Session() == null)
			return null;
		
		List list = getDomainFactory().find(" select theatreDet from Sch_Session as session left join session.sch_Profile as profile left join profile.theatreDetails as theatreDet where session.id = :sessionID ", new String[] {"sessionID"}, new Object[] {session.getID_Sch_Session()});
			
		return TheatreDetailVoAssembler.createTheatreDetailVoCollectionFromTheatreDetail(list);
