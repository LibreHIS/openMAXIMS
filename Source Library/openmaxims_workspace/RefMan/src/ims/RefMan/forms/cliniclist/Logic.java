...
import ims.scheduling.vo.BookingAppointmentClinicListListVo;
...
public class Logic extends BaseLogic
{
	...
	protected void onFormOpen(Object[] args) throws PresentationLogicException
	{
		...
		if (form.getGlobalContext().RefMan.getClinicListFilterIsNotNull())
		{
			...
			form.cmbListType().setValue(form.getGlobalContext().RefMan.getClinicListFilter().getListType());	//wdev-19419
	...
	protected void onBtnClearClick() throws ims.framework.exceptions.PresentationLogicException
	{
		...
		form.cmbListType().clear();	//wdev-19419
	...
	private void updateBooking(Status_Reason stat, AppointmentTrackingStatus trackingStatus, boolean requiresNewTCI)
	{
		...
		if(form.grdSlots().getValue() instanceof SessionSlotClinicListListVo)
		{
    		...
    		else if (stat.equals(Status_Reason.BOOKED))
    		{
    			...
    			if (AppointmentTrackingStatus.UNDO_ARRIVAL.equals(trackingStatus))
    				isUndoArrival = true;
    		}
    		...
    		try 
    	 	{
    			domain.saveAppointmentForDNA(voBooking, voCats, sessionSlot, patientElectiveList, isFirstApptActivityforDNA, isUndoArrival); //WDEV-18325
    		} 
	...
	protected void onFormDialogClosed(FormName formName, DialogResult result) throws PresentationLogicException 
	{
		...
		else if(formName.equals(form.getForms().Scheduling.CancelAppointmentDialog)) //WDEV-12568
		{
			if(result.equals(DialogResult.OK))
			{
				...
				{	
					vo = domain.getCatsReferral(form.getGlobalContext().RefMan.getCatsReferral());
				}
				form.getGlobalContext().RefMan.setReferralContractTypeForPatient(vo != null && vo.getContractIsNotNull() ? vo.getContract().getContractType() : null);
				...
				if(form.getGlobalContext().Scheduling.getApptCancelStatusIsNotNull() && form.getGlobalContext().Scheduling.getApptCancelStatus().getRebookSelectedIsNotNull() && form.getGlobalContext().Scheduling.getApptCancelStatus().getRebookSelected())
				{
					...
					engine.open(form.getGlobalContext().RefMan.getCatsReferral() != null ? form.getForms().RefMan.BookAppointment : form.getForms().Scheduling.BookAppointment); //WDEV-20101 if no referral open Scheduling Booking Appointment form.
					return;
				}
				else
				{
...
//		dna();
	...
	private void setFormReadOnly(Boolean value) 
	{
		...
		form.cmbListType().setEnabled(!value);	//wdev-19419
...
//		wdev-12682
	...
	private void performSearch() 
	{
		...
		if(validateSearch())
		{
			...
			voFilter.setListType(form.cmbListType().getValue());	//wdev-19419
	...
	private void populateSlots(	SessionSlotCatsReferralClinicListWrapperVoCollection voCollSessionSlots) 
	{
		...
		for (SessionSlotCatsReferralClinicListWrapperVo slot: voCollSessionSlots)
		{
			...
			String cancelTooltip = getCancellationTooltip(item != null ? item.getAppointment() : null);
			
			if(item.getPriorityIsNotNull())
			{
				row.setcolPriority(item.getPriority().getText());
				row.setTooltipForcolPriority(cancelTooltip);
			}
			
			if(item.getStatusIsNotNull())
			{
				row.setcolSlotStatus(item.getStatus().getText());
				row.setTooltipForcolSlotStatus(cancelTooltip);
			...
			if(item.getStartTmIsNotNull())
			{
				...
				row.setTooltipForcolTime(cancelTooltip);
			}
			...
			//WDEV-19381
			if (item!=null && item.getAppointment()!=null && Boolean.TRUE.equals(item.getAppointment().getWasPrinted()))
			{	
				
				row.setcolAppLetterPrinted(form.getImages().Correspondence.Printer);
				row.setTooltipForcolAppLetterPrinted(getTooltipForColAppLetterPrintedForSlots(item.getAppointment()));
			}
		}
		
	}

	private String getCancellationTooltip(BookingAppointmentClinicListListVo appointment)
	{
		if(appointment == null)
			return null;
	...
	//WDEV-19381
	private String getTooltipForColAppLetterPrintedForSlots(BookingAppointmentClinicListListVo bookingAppointmentClinicListListVo)
	{
		if (bookingAppointmentClinicListListVo==null || !Boolean.TRUE.equals(bookingAppointmentClinicListListVo.getWasPrinted()))
		{
			return null;
		}
		...
		str+=(bookingAppointmentClinicListListVo.getBookingLetterIsNotNull() && bookingAppointmentClinicListListVo.getBookingLetter().getRecordingUser()!=null ? "<br><b>User:  </b>" + bookingAppointmentClinicListListVo.getBookingLetter().getRecordingUser().getIMosName() : "");  
		
		str+=(bookingAppointmentClinicListListVo.getBookingLetterIsNotNull() && bookingAppointmentClinicListListVo.getBookingLetter().getRecordingDateTime()!=null ? "<br><b>Date/Time: </b>" + bookingAppointmentClinicListListVo.getBookingLetter().getRecordingDateTime() : "");  
			...
			if(voBooking.getAppointmentsIsNotNull())
			{
				for(int p=0;p<voBooking.getAppointments().size();p++)
				{
					...
					{	
						...
						row.setcolAppLetterPrinted(form.getImages().Correspondence.Printer);
						row.setTooltipForcolAppLetterPrinted(getTooltipForColAppLetterPrintedForBooking(voAppt));
					}
	...
	//WDEV-19381
	private String getTooltipForColAppLetterPrintedForBooking(Booking_AppointmentVo  bookAppt)
	{
	...
	{
		if (form.getGlobalContext().RefMan.getCatsReferral() == null) //WDEV-20101
		{
			...
			try
			{
				...
				if(form.getGlobalContext().Scheduling.getApptCancelStatusIsNotNull())
				{
					...
					if(Status_Reason.HOSPITALCANCELLED.equals(form.getGlobalContext().Scheduling.getApptCancelStatus().getStatusReason()))
					{
						...
						if(voSessionSlot.getAppointment().getNumProviderCancellations() != null)
						{
							numProviderCancellations = voSessionSlot.getAppointment().getNumProviderCancellations();
						}
						numProviderCancellations += 1;

						voSessionSlot.getAppointment().setNumProviderCancellations(numProviderCancellations);
					}
					...
					if(Status_Reason.PATIENTCANCELLED.equals(form.getGlobalContext().Scheduling.getApptCancelStatus().getStatusReason()))
					{
						...
						if(voSessionSlot.getAppointment().getNumPatientCancellations() != null)
						{
							numPatientCancellations = voSessionSlot.getAppointment().getNumPatientCancellations();
						}
						numPatientCancellations += 1;

						voSessionSlot.getAppointment().setNumPatientCancellations(numPatientCancellations);
					}
				}
				...
				//wdev-8288
				if (form.getGlobalContext().RefMan.getCatsReferral() != null) //WDEV-20101  if referral reference is null NPE fix
				{	
					voRef = domain.getCatsReferral(form.getGlobalContext().RefMan.getCatsReferral());
				...
				if (voRef != null) //WDEV-20101 
				{	
					domain.updateCatsReferralAdditionalInvStatus(voRef, voSessionSlot.getAppointment());
				}
	...
	private Appointment_StatusVoCollection getApptStatusHistory(SessionSlotVo voSessionSlot, Status_Reason status, Appointment_StatusVo voApptStatus) 
	{
		...
		voApptStatus.setSession(voSessionSlot.getAppointment().getSessionIsNotNull() ? voSessionSlot.getAppointment().getSession() : null); //WDEV-23185
