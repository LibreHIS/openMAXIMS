...
import ims.clinical.vo.DiagnosisComplicationSearchCriteriaVo;
...
import ims.core.vo.lookups.DiagnosisStatus;
...
public class Logic extends BaseLogic
{
	...
	protected void onFormOpen() throws ims.framework.exceptions.PresentationLogicException
	{
		...
		//WDEV-19389 
		if (!(form.getGlobalContext().Core.getPatientShortIsNotNull() && form.getGlobalContext().Clinical.getDiagnosisComplicationSearchCriteriaIsNotNull() && form.getGlobalContext().Clinical.getDiagnosisComplicationSearchCriteria().getPatientIsNotNull() && form.getGlobalContext().Core.getPatientShort().equals(form.getGlobalContext().Clinical.getDiagnosisComplicationSearchCriteria().getPatient())))
			form.getGlobalContext().Clinical.setDiagnosisComplicationSearchCriteria(null);
		
		if(form.getGlobalContext().Clinical.getDiagnosisComplicationSearchCriteriaIsNotNull())
		{
			setSearchCriteria(form.getGlobalContext().Clinical.getDiagnosisComplicationSearchCriteria());
		}
		//WDEV-19389 - end
	...
	private DiagnosisComplicationSearchCriteriaVo getSearchCriteria()
	{
		DiagnosisComplicationSearchCriteriaVo searchCriteria = new DiagnosisComplicationSearchCriteriaVo();
		
		searchCriteria.setExcludeResolved(Boolean.TRUE.equals(form.chkExcludeResolved().getValue()) ? true : false);
	...
	private void setSearchCriteria(DiagnosisComplicationSearchCriteriaVo diagnosisComplicationSearchCriteriaVo)
	{
		form.chkExcludeResolved().setValue(diagnosisComplicationSearchCriteriaVo.getExcludeResolved());	
	}
	...
	public void initialize() throws ims.framework.exceptions.FormOpenException
	{
		...
		excludeNegativeInstancesForCmbStatus();//WDEV-22906
		setDateValidation();
	...
	private void excludeNegativeInstancesForCmbStatus()
	{
		for(int i=0; form.ctnDetails().cmbStatus().getValues()!= null && i<form.ctnDetails().cmbStatus().getValues().size();i++)
		{
			DiagnosisStatus rowValue =  (DiagnosisStatus)form.ctnDetails().cmbStatus().getValues().get(i);
			if(DiagnosisStatus.EXCLUDED.equals(rowValue))
				form.ctnDetails().cmbStatus().removeRow(rowValue);
		}
	}
	...
	private void setHcpSearchComponentsSettings()
	{
		...
		form.ctnDetails().customControlMos().initialize(MosType.HCP);
	}
	...
	private void populateInstanceControls(PatientDiagnosisVo voPatDiag)
	{
		...
		*/
		...
		form.ctnDetails().chkDiagnosedOnAdmission().setValue(voPatDiag.getDiagnosedOnAdmissionIsNotNull() ? voPatDiag.getDiagnosedOnAdmission() : false); //WDEV-19860
	...
	private PatientDiagnosisVo populateDataFromScreen()
	{
		...
		voPatDiagnosis.setDiagnosedOnAdmission(form.ctnDetails().chkDiagnosedOnAdmission().getValue()); //WDEV-19860
	...
	public void clearInstanceControls()
	{
		...
		form.ctnDetails().chkDiagnosedOnAdmission().setValue(false); //WDEV-19860
	...
	private boolean saveDiagnosisRecord(PatientDiagnosisVo voPatientDiagnosis, Boolean savePmh, String[] uiErrors) throws PresentationLogicException
	{
		...
		PatientNoDiagInfoForCCVo patientNoDiagInfo = populatePatientNoDiagInfo(form.getGlobalContext().Core.getCurrentCareContext(), form.getGlobalContext().Core.getPatientShort(),voPatientDiagnosis); 
		
		try
		{
			voPatientDiagnosis = domain.saveDiagnosisVo(voPatientDiagnosis, form.getGlobalContext().Core.getPatientShort(), savePmh, patientNoDiagInfo);
	...
	private PatientNoDiagInfoForCCVo populatePatientNoDiagInfo(CareContextShortVo careContext, PatientShort patientShort, PatientDiagnosisVo diag)
	{
		if (careContext == null)
			return null;

		boolean isDiagnosis = (diag != null && !Boolean.TRUE.equals(diag.getIsComorbidity()) && !Boolean.TRUE.equals(diag.getIsComplication()));
		...
		if (isDiagnosis)
		{
			...
			if (PatientDiagnosisStatus.HASDIAGNOSISFOREPISODE.equals(patientNoDiagInfo.getDiagnosisStatus()))
				return null;
	...
	{
		PatientDiagnosisVo selectedPatientDiagnosisVo = form.getLocalContext().getSelectedPatientDiagnosisVo();
		boolean bAddnew = form.getMode().equals(FormMode.EDIT) && (selectedPatientDiagnosisVo == null || selectedPatientDiagnosisVo.getID_PatientDiagnosis() == null);
		...
		if (form.getMode().equals(FormMode.EDIT))
		{
			...
			form.ctnDetails().chkDiagnosedOnAdmission().setEnabled(bAddnew); //WDEV-19860
		}
		...
		boolean excludeNegInstance = selectedPatientDiagnosisVo != null && selectedPatientDiagnosisVo.getCurrentStatusIsNotNull() && DiagnosisStatus.EXCLUDED.equals(selectedPatientDiagnosisVo.getCurrentStatus().getStatus());
		form.ctnDetails().cmbStatus().setEnabled(FormMode.EDIT.equals(form.getMode()) && !excludeNegInstance);
	...
	public void updateInstance()
	{
		...
		if(!DiagnosisStatus.EXCLUDED.equals(form.ctnDetails().cmbStatus().getValue()))
			excludeNegativeInstancesForCmbStatus();//WDEV-22906
		updateContextMenus();
	}
	...
	protected void onChkExcludeResolvedValueChanged() throws PresentationLogicException
	{
		...
		form.getGlobalContext().Clinical.setDiagnosisComplicationSearchCriteria(getSearchCriteria());//WDEV-19389
	}
	...
	protected void onRIEDialogClosed(DialogResult result) throws PresentationLogicException
	{
		if (result.equals(DialogResult.OK))
		{
			...
			//WDEV-18627
			PatientNoDiagInfoForCCVo patientNoDiagInfo = populatePatientNoDiagInfoForRie(form.getGlobalContext().Core.getCurrentCareContext(), form.getGlobalContext().Core.getEpisodeofCareShort(), form.getGlobalContext().Core.getPatientShort());

			if (patientNoDiagInfo != null)
			{
				try
				{
					domain.savePatientNoDiagInfo(patientNoDiagInfo);
				}
				catch (StaleObjectException e)
				{
					engine.showMessage(ims.configuration.gen.ConfigFlag.UI.STALE_OBJECT_MESSAGE.getValue());
