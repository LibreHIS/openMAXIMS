...
public class Logic extends BaseLogic
{
	...
	protected void onFormOpen(Object[] args) throws ims.framework.exceptions.PresentationLogicException
	{
		...
		form.lblDeathNotificationStatus().setVisible(isPds());
		form.cmbDeathNotificationStatus().setVisible(isPds());
		form.getContextMenus().Clinical.hideAllDeathDetailsMenuItems();

		if (FormMode.VIEW.equals(form.getGlobalContext().Core.getParentFormMode()))
		{
			form.setMode(FormMode.VIEW);			
		}

		form.btnUndoDateofdeath().setVisible(FormMode.VIEW.equals(form.getMode()) && form.dteDOD().getValue() != null);

		if(/*engine.hasRight(AppRight.PDS_ACCESS_UNDECEASE) && */form.dteDOD().getValue() != null && FormMode.VIEW.equals(form.getMode()))
		{
			try
			{
				form.getLocalContext().setPatient(domain.getPatient(form.getGlobalContext().Core.getPatientShort()));
			}
			catch (StaleObjectException e)
			{
				e.printStackTrace();
				engine.showMessage("Error getting the patient record: " + e.toString());
			}
		}
	...
	protected void onBtnSaveClick() throws ims.framework.exceptions.PresentationLogicException
	{
		...
		ddVo.getPatient().setTimeOfDeath(form.timTOD().getValue() != null ? form.timTOD().getValue() : null);
	...
	private String[] getUIErrors() 
	{
		...
		{
			errors.add("Date of Death cannot be set to a date in the future.");
		}		
		if (form.timTOD().getValue() != null && form.dteDOD().getValue() == null)
		{
			errors.add("Date of Death is required when Time of Death is set.");
			form.dteDOD().setFocus();
		}
		if (form.timTOD().getValue() != null && form.dteDOD().getValue() != null && new DateTime(form.dteDOD().getValue(), form.timTOD().getValue()).isGreaterThan(new DateTime()))
		{
			errors.add("Time of Death cannot be later than current time.");
		}
		if (errors.size() > 0) 
		{			
	...
	@Override
	protected void onBtnUndoDateofdeathClick() throws PresentationLogicException
	{
		Patient patient = form.getLocalContext().getPatient();
	
		patient.setDod(null);
		patient.setTimeOfDeath(null);
		...
		try
		{
			patient = domain.unDeceasePatient(patient, ddVo);
		}
		catch (StaleObjectException e)
		{
		...
		form.getGlobalContext().Core.setRefreshParent(Boolean.TRUE);
		engine.close(DialogResult.OK);
	}
