...
import ims.clinical.vo.DischargeMedicationDetailsLiteVoCollection;
...
import ims.clinical.vo.domain.DischargeMedicationDetailsLiteVoAssembler;
...
public class EDischargePreViewComponentImpl extends BaseEDischargePreViewComponentImpl
{
	...
	//WDEV-18635
	public DischargeMedicationDetailsLiteVo getDischargeMedicationDetailsForCareContext(CareContextRefVo careContext)
	{
		if(careContext == null ||
				careContext.getID_CareContext() == null)
			throw new CodingRuntimeException("CareContext is null or id not provided for getDischargeDetailsForCareContext");	
		...
		DischargeMedicationDetailsLiteVoCollection medication = DischargeMedicationDetailsLiteVoAssembler.createDischargeMedicationDetailsLiteVoCollectionFromMedicationDetails(dischargeMedication);		
	...
	public DischargeReportDetailVo saveDischargeReportDetail(DischargeReportDetailVo record, DischargeReadyToLeaveVo voReady, RTLSummaryVo rTLSummaryVo, PatientNoDiagInfoForCCVo patientNoDiagInfo) throws StaleObjectException
	{
		if (record != null)
		{
	...
	public PatientDiagnosisEDischargeListVoCollection listComorbidity(
			EpisodeOfCareRefVo episodeofCareRefVo, CareContextRefVo careContextRefVo) {
		...
		return impl.listIncludedComorbidities(episodeofCareRefVo, careContextRefVo); //WDEV-19820
		
	}
	
	//WDEV-20225
	public PatientDiagnosisEDischargeListVoCollection listRelevComorbidity(
			EpisodeOfCareRefVo episodeOfCareRefVo, CareContextRefVo careContextRefVo) {
		EDischargeDiagnosisEtcComponent impl = (EDischargeDiagnosisEtcComponent)getDomainImpl(EDischargeDiagnosisEtcComponentImpl.class);
		return impl.listRelevantComorbidities(episodeOfCareRefVo, careContextRefVo); 
	}	
	//----------------
	...
	//WDEV-18622
	public RTLSummaryVo getRTLSummaryForCareContext(CareContextRefVo careContextRef)
	{
		IEDischargeHelper eDischImpl = (IEDischargeHelper)getDomainImpl(EDischargeHelper.class);
		return eDischImpl.getRTLSummaryForCareContext(careContextRef);
	}

	public PatientNoDiagInfoForCCVo getPatientNoDiagInfoForCareContext(CareContextRefVo careContextRef)
	{
		EDischargeDiagnosisEtcComponent eDischargeImpl = (EDischargeDiagnosisEtcComponent) getDomainImpl(EDischargeDiagnosisEtcComponentImpl.class);
		return eDischargeImpl.getPatientNoDiagInfoForCareContext(careContextRef);
	}

	public PatientDiagnosisEDischargeListVoCollection listDiagnoses(EpisodeOfCareRefVo episodeofCareRefVo)
	{
		EDischargeDiagnosisEtcComponent impl = (EDischargeDiagnosisEtcComponent)getDomainImpl(EDischargeDiagnosisEtcComponentImpl.class);
		return impl.listDiagnosis(episodeofCareRefVo);
	}
