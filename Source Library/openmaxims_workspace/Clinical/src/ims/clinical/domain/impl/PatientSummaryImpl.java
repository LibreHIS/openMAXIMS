...
//#############################################################################
//#EOH
// This code was generated by Daniel Laffan using IMS Development Environment (version 1.44 build 2245.28406)
...
import ims.clinical.domain.base.impl.BasePatientSummaryImpl;
...
public class PatientSummaryImpl extends BasePatientSummaryImpl
{
	...
	public CareSpellVoCollection listCareSpellsByPatientAndCriteria(PatientRefVo patientRef, Date dateFrom, Date dateTo, Integer context,Boolean showCancelled) 
	{
		...
		if(context == 0 )			//search against carespell
		{
			...
			//-----
			//careSpells = factory.find("from CareSpell careSpell where careSpell.patient.id = :idPatient order by careSpell.systemInformation.creationDateTime desc", new String[] {"idPatient"}, new Object[] {patientRef.getID_Patient()});
		}
		else if(context == 1)		//search against carespell with date from and date to
		{
			...
			//-----
			//careSpells = factory.find("from CareSpell careSpell where careSpell.patient.id = :idPatient and careSpell.startDate between :datefrom and :dateto order by careSpell.systemInformation.creationDateTime desc", new String[] {"idPatient","datefrom","dateto"}, new Object[] {patientRef.getID_Patient(),dateFrom.getDate(),dateTo.getDate()});
		}
		else if(context == 2)		//search against episodeofcare with date from and date to	
		{
			//careSpells = factory.find("select c1_1,e1_1 from CareSpell as c1_1 left join c1_1.episodes as e1_1 left join c1_1.patient as p1_1 where (e1_1.startDate between :datefrom and :dateto and p1_1.id = :idPatient) order by c1_1.systemInformation.creationDateTime desc,e1_1.systemInformation.creationDateTime desc " , new String[] {"idPatient","datefrom","dateto"}, new Object[] {patientRef.getID_Patient(),dateFrom.getDate(),dateTo.getDate()});
			//wdev-13977
			ArrayList names = new ArrayList();
			ArrayList values = new ArrayList();
			
			StringBuffer hql = new StringBuffer("select c1_1, e1_1 from CareSpell as c1_1 left join c1_1.episodes as e1_1 left join e1_1.currentStatus as e2_1 left join c1_1.patient as p1_1 left join c1_1.currentStatus as c2_1"); //, ClinicalContactStatusHistory as c3_1
		...
		else if(context == 3)		//search against carecontext with date from and date to
		{
			...
			//careSpells = factory.find("select c1_1,e1_1,c2_1 from CareSpell as c1_1 left join c1_1.episodes as e1_1 left join e1_1.careContexts as c2_1 where (c2_1.startDateTime  between :datefrom and :dateto and c1_1.patient.id = :idPatient) order by c1_1.systemInformation.creationDateTime desc,e1_1.systemInformation.creationDateTime desc , c2_1.systemInformation.creationDateTime desc" , new String[] {"idPatient","datefrom","dateto"}, new Object[] {patientRef.getID_Patient(),dateFrom.getDate(),dtDT});
			ArrayList names = new ArrayList();
			ArrayList values = new ArrayList();
	...
	public String getPIDDiagnosisInfo(CareContextRefVo careContextRefVo, EpisodeOfCareRefVo episodeRefVo, CareSpellRefVo careSpellRefVo)
	{
		...
		//WDEV-17339
		if (Boolean.FALSE.equals(ConfigFlag.UI.DISPLAY_PRIMARY_DIAGNOSIS_IN_PID.getValue()))
			return "";
	...
	public Boolean hasActiveEpisodeOfCare(CareSpellRefVo careSpell)
	{
		...
		paramNames.add("CARE_SPELL_ID");		paramValues.add(careSpell.getID_CareSpell());
		paramNames.add("OPEN_STATUS");			paramValues.add(EpisodeOfCareStatus.OPEN.getID());
