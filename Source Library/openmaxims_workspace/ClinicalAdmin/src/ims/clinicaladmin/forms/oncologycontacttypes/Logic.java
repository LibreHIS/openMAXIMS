...
public class Logic extends BaseLogic
{
	...
	private void selectionChanged() 
	{
			...
			&& form.treList().getSelectedNode().getValue() instanceof ContactTypesByDisciplineItemVo)
		{
			...
			//WDEV-20460
			ContactTypesByDisciplineItemVo contactTypesByDisciplineItem = domain.getContactTypesByDisciplineItem((ContactTypesByDisciplineItemRefVo) form.treList().getSelectedNode().getValue());
			form.getLocalContext().setSelectedItem(contactTypesByDisciplineItem);
			populateInstanceControls(contactTypesByDisciplineItem);
		}
		else
		{
			clearInstanceControls();
	...
	private void reOpen() throws PresentationLogicException
	{
		...
		if(form.getLocalContext().getSelectedItem() == null && form.treList().getValue() instanceof ContactTypesByDisciplineItemVo)
		{
			form.getLocalContext().setSelectedItem((ContactTypesByDisciplineItemVo) form.treList().getValue());
		}
		...
		clearInstanceControls();
			...
			if (form.treList().getSelectedNode() != null && form.treList().getSelectedNode().getValue() instanceof ContactTypesByDisciplineItemVo)
			{	
				ContactTypesByDisciplineItemVo contactTypesByDisciplineItem = domain.getContactTypesByDisciplineItem((ContactTypesByDisciplineItemRefVo) form.treList().getSelectedNode().getValue());
				form.getLocalContext().setSelectedItem(contactTypesByDisciplineItem);
				populateInstanceControls(contactTypesByDisciplineItem);
	...
	private void populateInstanceControls(ContactTypesByDisciplineItemVo selectedRecord) 
	{
		if (selectedRecord != null)
		{
			...
			//WDEV-20381
			if(selectedRecord.getTaxonomyMapIsNotNull())
			{
				form.ctnDetails().ccCodeMapping().setValue(selectedRecord.getTaxonomyMap());
			}
			...
			{
				//WDEV-20381
				form.ctnDetails().lblParent().setValue(limitlblParentLength(form.treList().getNodeByValue(selectedRecord.getParent()).getText()));
				...
				form.ctnDetails().lblParent().setValue(form.treList().getNodeByValue(selectedRecord) != null 
					...
					&& form.treList().getNodeByValue(selectedRecord).getParent() != null ? limitlblParentLength(form.treList().getNodeByValue(selectedRecord).getParent().getText()) : "");//WDEV-20381
		}
	}
	
	//WDEV-20381
	private String limitlblParentLength(String parent)
	{
		form.ctnDetails().lblParent().setTooltip(null);
		form.ctnDetails().lblParent().setTooltip(parent);
		if(parent.length()>30)
		{
			parent = parent.substring(0, 27) + "...";
			return parent;
		}
		return parent;
	}
	...
	private void newContact()
	{
		...
		if (form.treList().getSelectedNode().getValue() != null && form.treList().getSelectedNode().getValue() instanceof ContactTypesByDisciplineItemVo)
			form.ctnDetails().lblParent().setValue(limitlblParentLength(form.treList().getSelectedNode().getParent().getText()));//WDEV-20381
		else
			form.ctnDetails().lblParent().setValue(limitlblParentLength(form.treList().getSelectedNode().getText()));//WDEV-20381
	...
	public void clearInstanceControls()
	{
		...
		form.ctnDetails().ccCodeMapping().clear();//WDEV-20381
	...
	{
		//WDEV-20460 - start
		ContactTypesByDisciplineVo voContact = (ContactTypesByDisciplineVo)form.getLocalContext().getSelectedRecord().clone();
		ContactTypesByDisciplineVo saveContactTypeByDiscipline = voContact;
		
		if ( !validateUI())
			return false;
		
		if(voContact.getID_ContactTypesByDiscipline() == null)
		{
			voContact.setDiscipline(form.ctnDetails().cmbDiscipline().getValue());
		}
		...
		if(voContact.getContactTypes().contains(voContactItem))
		{
			...
			voContact.getContactTypes().get(index).setActive(voContactItem.getActive());
			voContact.getContactTypes().get(index).setParent(voContactItem.getParent());
			voContact.getContactTypes().get(index).setContactType(voContactItem.getContactType());
			voContact.getContactTypes().get(index).setTaxonomyMap(voContactItem.getTaxonomyMap());
		}
		...
		try
		{
			saveContactTypeByDiscipline = domain.saveContactTypeByDiscipline(voContact);	
		}
		catch(StaleObjectException e)
		{
			engine.showMessage(ims.configuration.gen.ConfigFlag.UI.STALE_OBJECT_MESSAGE.getValue());
		...
		form.getLocalContext().setSelectedRecord(saveContactTypeByDiscipline);
		int index = saveContactTypeByDiscipline.getContactTypes().indexOf(voContactItem);
		if(index!=-1)
			form.getLocalContext().setSelectedItem(saveContactTypeByDiscipline.getContactTypes().get(index));
		else
			form.getLocalContext().setSelectedItem(saveContactTypeByDiscipline.getContactTypes().get(saveContactTypeByDiscipline.getContactTypes().size()-1));
	...
	private void populateItemFromCollection(ContactTypesByDisciplineItemVo dest, ContactTypesByDisciplineItemVo src)
	{
		...
		//WDEV-20381
		if(form.ctnDetails().ccCodeMapping().getValueNoWarnings()!= null)
			dest.setTaxonomyMap(form.ctnDetails().ccCodeMapping().getValueNoWarnings());
	...
	private ContactTypesByDisciplineItemVo populateInstanceData() 
	{
		...
		//WDEV-20381
		if(form.ctnDetails().ccCodeMapping().getValueNoWarnings()!= null)
			voItem.setTaxonomyMap(form.ctnDetails().ccCodeMapping().getValueNoWarnings());
	...
	private void formModeChanged()
	{
		...
		form.ctnDetails().ccCodeMapping().setComponentMode(form.getMode());//WDEV-20381
	}
