...
public final class EDDischargeDetailsJobImpl extends ims.scheduler.SchedulerJob
{
	...
	private static Integer	 ED_DISCHARGE_SUMMARY_REPORT_ID = 346;
	...
	public SchedulerJobExecutionSummary doExecute() throws JobExecutionException
	{
		...
		EDDischargeSummaryScheduleVo job = getJobDetails();
		if (job == null)
		{
		...
		TrackingJobVoCollection dischargedTrackings = getDischargedTrackings();
		
		try
		{
			savePatientDocuments(dischargedTrackings, job);
		}
		catch(Exception e)
		{
	...
	private synchronized void saveJob(EDDischargeSummaryScheduleVo job)
	{
		if(job == null)
			return;
		...
		EDDischargeSummarySchedule doJob = EDDischargeSummaryScheduleVoAssembler.extractEDDischargeSummarySchedule(factory, job);
		try 
		{
			factory.save(doJob);
		} 
	...
	private EDDischargeSummaryScheduleVo getJobDetails() 
	{
		EDDischargeSummarySchedule doEDDischargeSummarySchedule = EDDischargeSummarySchedule.getEDDischargeSummaryScheduleFromConfiguredJobDetails(getDomainFactory(), super.getConfiguredJob().getConfiguredScheduledJobID());
		return EDDischargeSummaryScheduleVoAssembler.create(doEDDischargeSummarySchedule);
	}

	private synchronized void savePatientDocuments(TrackingJobVoCollection dischargedTrackings, EDDischargeSummaryScheduleVo job) throws Exception 
	{
		...
		Object[] obj = getSystemReportAndTemplate(ED_DISCHARGE_SUMMARY_REPORT_ID);
		...
		{
			createPatientDocument(tracking, obj[0], obj[1], disableEmail);
		}
	}

	private void createPatientDocument(TrackingJobVo tracking, Object report, Object template, boolean disableEmail) throws Exception 
	{
		if(tracking == null)
			return;
		...
		try
		{
			document = saveDocument(tracking);
			if(document == null)
				return;
		...
		client.addSeed(new SeedValue("CareContext_id",  tracking.getAttendance().getCareContext().getID_CareContext(), Integer.class)); //wdev-22416
		...
		{							
			...
			long end = System.currentTimeMillis();
			trace("Report build successful in :" + (end - start) + " ms" );
		...
		{	
			...
			trace("Build report failed! " + err.getMessage());
			return;
		}
	...
	private void rollback(PatientDocumentVo document, TrackingJobVo tracking) throws StaleObjectException 
	{
		if(tracking != null)
		{
			tracking.setDischargeLetterStatus(DischargeLetterStatus.IN_PROGRESS);
	...
	private PatientDocumentVo saveDocument(TrackingJobVo tracking) throws StaleObjectException 
	{
		PatientDocumentVo document = populatePatientDocument(tracking);
	...
	private TrackingJobVo populateAndSaveTracking(TrackingJobVo tracking) throws StaleObjectException 
	{
		...
		tracking.setDischargeLetterStatus(DischargeLetterStatus.GENERATED);
	...
	private boolean sendMailToPatientGP(PatientForEDDischargeVo patient) throws JobExecutionException 
	{
		...
		String 	emailMsgTxt 	= "ED Discharge Summary Report is attached";
	...
	private PatientDocumentVo populatePatientDocument(TrackingJobVo tracking) 
	{
		if(tracking == null)
			return null;
	...
	public void cleanUpSettings(IConfiguredScheduledJob job) throws Exception
	{
		...
		if(doEDDischargeSummarySchedule != null)
		{
			getDomainFactory().delete(doEDDischargeSummarySchedule);
		}
	}
