...
public class Logic extends BaseLogic
{
	...
	protected void onBtnSearchClick() throws ims.framework.exceptions.PresentationLogicException
	{
		...
		if(orgVo == null)
		{
			engine.showMessage("No location records were found that match the specified search criteria.", "No Data Found", MessageButtons.OK, MessageIcon.WARNING); //WDEV-18801
			return;
		}
		else
		{
	...
	private  void getLocationsFromOrganisatio(OrganisationConfigVo orgVo,ConfigLocationLiteVoCollection tempVoColl,String name)
	{
		...
		{	
			if(name == null || checkCharacter(name))
			{
				...
				{
					if(orgVo.getLocationSites().get(i) != null && Boolean.TRUE.equals(orgVo.getLocationSites().get(i).getIsActive()) && Boolean.FALSE.equals(orgVo.getLocationSites().get(i).getIsVirtual())) //WDEV-19532
						tempVoColl.add(orgVo.getLocationSites().get(i));
					//addLocation(orgVo.getLocationSites().get(i),tempVoColl); WDEV-20054 -- no longer lists locations
			...
			else if(checkBegin(name))   //check if location name begin with % and not end with %
			{
				...
				for(int i = 0;i < orgVo.getLocationSites().size();i++)
				{
					...
					{
						if(Boolean.TRUE.equals(orgVo.getLocationSites().get(i).getIsActive()) && Boolean.FALSE.equals(orgVo.getLocationSites().get(i).getIsVirtual()))	 //WDEV-19532
							tempVoColl.add(orgVo.getLocationSites().get(i));
					}
					//addLocationBegin(orgVo.getLocationSites().get(i),tempVoColl,nameloc1); WDEV-20054 -- no longer lists locations
			...
			else if(checkEnd(name)) //check if location name end with %
			{
				...
				for(int i = 0;i < orgVo.getLocationSites().size();i++)
				{
					...
					//addLocationEnd(orgVo.getLocationSites().get(i),tempVoColl,nameloc1); WDEV-20054 -- no longer lists locations
			...
			else if(checkBeginEnd(name))//check if location name begin with % and end with %
			{
				...
				for(int i = 0;i < orgVo.getLocationSites().size();i++)
				{
					...
					//addLocationContain(orgVo.getLocationSites().get(i),tempVoColl,nameloc1); WDEV-20054 -- no longer lists locations
			...
			else
			{
				...
				for(int i = 0;i < orgVo.getLocationSites().size();i++)
				{
					...
					//addLocationContain(orgVo.getLocationSites().get(i),tempVoColl,name); WDEV-20054 -- no longer lists locations
