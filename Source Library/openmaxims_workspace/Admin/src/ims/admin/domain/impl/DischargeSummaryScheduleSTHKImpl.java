...
public class DischargeSummaryScheduleSTHKImpl extends BaseDischargeSummaryScheduleImpl
{
	...
	{	
		...
		try 
		{
			ims.admin.domain.DischargeSummarySchedule impl = (ims.admin.domain.DischargeSummarySchedule)DomainImplFlyweightFactory.getInstance().create(DischargeSummaryScheduleSTHKImpl.class, ims.domain.DomainSession.getSession());			
		...
		for (int i = 0; i < summaries.size(); i++) 
		{
				...
				&& summaryVo.isSummaryRecordComplete())
			{
				...
				if (category.equals(DocumentCategory.DISCHARGE_SUMMARY) 
					&& summaryVo.getDischargeLetterStatusIsNotNull() 
					&& summaryVo.getDischargeLetterStatus().equals(DischargeLetterStatus.GENERATED))
				{
					continue;
				}

				if (category.equals(DocumentCategory.DISCHARGE_SUPPLEMENTARY) 
					&& summaryVo.getDischargeSupplementaryLetterStatusIsNotNull() 
					&& summaryVo.getDischargeSupplementaryLetterStatus().equals(DischargeSupplementaryLetterStatus.GENERATED))
				{
					continue;
				}
				...
				GP gpVo = getGPViaCareContext(summaryVo.getCareContext());

				PatientDocumentVo document = null;
				try
				{
					document = populatePatientDocumentVo(populateServetDocumentVo(), summaryVo, docName, category);
				...
				{	
					...
					try
					{
						...
						new SendMail().sendSSLMessage(gpVo.getEmailAddress(), emailSubjectTxt,emailMsgTxt, pdfStorePath + filePath);									
						long end = System.currentTimeMillis();
